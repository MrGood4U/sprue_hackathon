...
The proposed frontend/backend HTTP contract is [api-contract.md](api-contract.md) Draft 0.2, with domain details under `docs/api/`. Review it before implementing frontend clients or backend routes. It records proposed authentication, DTOs, commands, concurrency, traces, private deployment, x402 delivery, and financial projections. Model directions M1-M3 (durable command idempotency, anonymous request recovery authorization, and validation/build/activation separation) were approved and incorporated into data-model 1.4. Database structure exists; HTTP guarantees, queue dispatch and service-level verification still require implementation and tests. Provider and capped-consumer gates E1/E2 remain explicit.
