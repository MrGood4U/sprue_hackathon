...
For downstream publication, use a resolved Hedera account ID as `payTo`, not an unresolved EVM-address string. Record account-ID/EVM-address mapping evidence, account completion, and per-asset receive/spend capability. HBAR uses entity ID `0.0.0` and tinybar units; HTS payments require a verified fungible-token ID, decimals, and association capability. Read Blocky402's current x402 v2 `exact` fee payer from `/supported`, pin it in each accepted requirement, and reconcile the facilitator transaction reference through Hedera Mirror Node before recognizing settlement.
