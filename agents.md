...
Use approved [product-design.md](product-design.md) version 1.0 as the page-architecture and interaction baseline. It defines seven page families, routes, navigation, each page's interaction elements, cross-page state transitions, loading/empty/error/financial states, desktop layout behavior, accessibility behavior, and approved data/API dependencies. The Creator Console targets desktop browsers at 1024 CSS pixels and above and is optimized for a 1440-pixel judge-demo viewport; mobile and tablet-specific layouts are deferred. Structured DAG controls remain required as a desktop keyboard and single-pointer alternative, not as a mobile editor. Do not infer new product behavior inside UI implementation when the design specification is silent; update the design record first.
