...
Before building application pages or components, obtain human approval for [product-design.md](product-design.md). Draft 0.1 proposes seven page families and defines routes, navigation, each page's interaction elements, cross-page state transitions, loading/empty/error/financial states, responsive and accessibility behavior, and approved data/API dependencies. It is not yet an implementation baseline. Do not infer new product behavior inside UI implementation when the design specification is silent; update the design record first.
