...
The Agent harness is documented in [backend/harness/README.md](backend/harness/README.md) Draft 0.7. It separates natural-language planning and verified source/query/operator compilation from creator-authorized deterministic execution. Its target orchestration uses three narrow model passes for semantic interpretation, inspected-source selection, and primitive-DAG composition, surrounded by deterministic discovery, query compilation, template expansion, stable-ID allocation, schema inference, validation, and canonical schemaVersion 2 assembly; [planner-orchestration.md](backend/harness/planner-orchestration.md) defines the Builder handoff. The bounded one-call mock and OpenAI-compatible model ports, proposal validation, and fixture-backed multi-source runtime are implemented; the multi-stage controller, live Graph execution, and durable planning remain pending. Consult its orchestration, workflow, tool/script catalog, operator semantics, constraints and verification plan before extending the planner/runtime. Tools are named typed functions, not unrestricted shell, code, network or wallet access. Only existing Subgraphs are eligible sources; source gaps lead to requirement revision or another existing-source candidate, never source creation or deployment. H2 persistence directions remain approved in model 1.6. H1 executable-language and H3 live-metric/operating-limit choices remain open; none enables payments.
