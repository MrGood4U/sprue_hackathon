...
Dashboard, Wallet and Access, Model Service, Agent Planner, Builder, API, and Monetize use owner-authorized live workspace APIs without a fixture fallback. The current local profile implements durable products, encrypted model/Graph credentials, Privy identity and wallet bootstrap, bounded existing-Subgraph discovery, durable Agent traces and Builder drafts, immutable live DAG compilation, fresh Graph execution, managed API deployment, Hedera testnet HBAR x402 publication, Blocky402 verification/settlement, and persisted paid-request/revenue evidence. Only the explicitly labeled public evaluator projection retains demo data. Account linking, distributed queue consumption, cross-instance planning cancellation recovery, production Privy additional-signer enforcement for upstream Graph x402 spending, and mainnet payment use remain outside the demonstrated slice. Starting all five services proves local infrastructure and configured application readiness; it does not prove a public cloud deployment. `DEMO_RUNTIME_ENABLED=true` affects only the temporary public evaluator path.
...
The downstream settlement profile is pinned to Hedera testnet. `HEDERA_NETWORK` must be `hedera:testnet`; `HEDERA_MIRROR_NODE_URL` defaults to `https://testnet.mirrornode.hedera.com`, and `BLOCKY402_FACILITATOR_URL` defaults to `https://api.testnet.blocky402.com`. The application derives chain ID 296 and native HBAR asset ID `0.0.0` from that profile. Configuration alone does not publish an endpoint or move funds: publication is an explicit owner command, and a buyer must separately approve and sign each paid request.
...
Verification results for this implementation are recorded in [plan.md](plan.md). Local container checks do not prove cloud deployment or replace the separately recorded live Graph/payment evidence. No wallet, payment, or Graph query is required for the infrastructure checks themselves.

The current development-host checks pass Docker startup/recreation for frontend, API, worker, PostgreSQL, and Redis; all 25 migrations apply with none pending; API/worker/database/cache health checks pass; and frontend deep links, missing-resource 404s, app configuration, and exact-origin CORS return the expected results. The migration suite verifies 55 tables and 741 columns, including durable Builder drafts, live deployments, immutable source targets, and paid-delivery records. The checked host uses frontend port 4174 because its local `.env.local` overrides the documented 4173 default. This is infrastructure verification, not a claim that the Vercel/Railway profile is currently deployed.
