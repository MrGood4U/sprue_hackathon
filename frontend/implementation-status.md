...
The Model Service page provides visible, validated fields for a full HTTPS OpenAI-compatible Chat Completions URL, an API key, and an exact model name. The key is sent once to the backend, never stored by the frontend, and never returned. The evaluator runtime retains the profile only in bounded process memory under a random browser-session identifier and clears it on API restart. The next explicit Agent plan uses the configured model; other actions do not trigger extra model calls. Durable workspace ownership, provider validation, rate limiting, and secret-manager persistence remain unimplemented.
...
The model-service implementation passed 40 backend tests, strict TypeScript/build/OpenAPI checks, the complete frontend build and Sites tests, deployment tests, and a local Docker image rebuild with all four services healthy. Browser verification covered the Chinese localized route, sidebar entry, large-screen layout, field-level validation with first-error focus, one-time key submission, redacted reload state, process-restart clearing, and an empty error/warning console. The next-plan model call is tested with an injected fake provider; no external model request or real API key was used.
