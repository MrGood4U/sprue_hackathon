...
Updated on 2026-09-06 after adding explicit Model Service connection testing.
...
| Model Service | Session-scoped OpenAI-compatible URL/key/model form with redacted backend reads, explicit non-saving connection test, and next-plan wiring | Verified creator identity, workspace-owned profile persistence, secret-manager storage/rotation, provider capability policy, usage limits, and recovery |
...
1. Review [api-contract.md](../api-contract.md) Draft 0.4 against approved data-model version 1.5 before generating typed contracts. M1-M3 and H2 persistence directions are approved and initial tables exist; M4 durable model-profile persistence remains under review. The offline DAG runtime exists, but business handlers, production identity verification, Graph adapters and queue processing remain unimplemented. A registered route returning 503 is not a usable frontend integration. Migrate the current JavaScript source incrementally to the selected TypeScript baseline and add real clients under `services/api/` as the backend becomes available.
