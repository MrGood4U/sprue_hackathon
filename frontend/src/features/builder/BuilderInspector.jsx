...
export function BuilderInspector({ selection, draft, onClose }) {
  ...
  const value = selection === "spec" ? spec : selection === "dag" ? spec.dag : selection === "schema" ? spec.outputSchema
    : spec.dag.nodes.find((node) => node.id === selection);
  const titleKey = selection === "spec" ? "builder.specTitle" : selection === "dag" ? "builder.structuredDag"
    : selection === "schema" ? "readiness.outputSchema" : nodeLabels[selection];
  return (
