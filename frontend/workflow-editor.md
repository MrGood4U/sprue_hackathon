...
Agent-authored GraphQL pushdown is projected as a residual workflow. Exact provider paths and selected fields remain inspectable on Source. Source preserves the raw nested provider shape, and its single project-mode Map boundary visibly performs every provider-to-semantic rename, flattening, unit annotation, conversion, and derivation. Mapping is never reported or removed as pushed work. An exactly equivalent Filter or Sort may be omitted and its predecessor reconnected to its successors, preventing duplicate execution while keeping the schema boundary explicit.
