...
Draft 0.18, updated on 2026-09-09 for human review. This is the target HTTP contract, not a claim that every durable endpoint is implemented. API, Monetize, and public evaluator views still use the explicitly temporary backend demo runtime documented in [demo-runtime.md](docs/api/demo-runtime.md). Dashboard, Wallet and Access, Model Service, Agent Planner, and the Builder planning-draft read use owner-authorized live workspace APIs and never fall back to browser fixtures. The Builder reads the newest durable Agent message and may project its server-owned `builderDraft`, but that object is explicitly non-executable and requires source admission; Builder edits remain browser-session-only and do not create versions. Dashboard product deletion is an implemented idempotent, ETag-protected tombstone command: ordinary product reads and new Agent work exclude the product while historical versions, runs, messages, evidence, financial records, and slug identity remain retained. Product creation accepts an empty initial intent only for the direct blank-draft-to-Agent flow; the first accepted non-empty Agent message initializes that product field. The current Agent slice persists real intent/messages/traces, appends sanitized planning events while the synchronous request is running, exposes the exact active command through authenticated session-scoped cursor polling, supports same-process cooperative cancellation, and invokes the saved model with bounded Graph metadata discovery, but stops at source feasibility until immutable source admission is implemented. Model and Graph API keys are encrypted before PostgreSQL storage and never returned. Privy creator authentication, provider-identity resolution to a stable Sprue user UUID, idempotent local account/workspace bootstrap, one user-owned Privy wallet per bootstrapped workspace, current Base Sepolia USDC balance reads, explicit Hedera testnet faucet activation/read, complete-account creator-control projection for the demonstrated Privy EVM path, creator-confirmed direct testnet withdrawals, and Graph-key validation/default selection/revocation are implemented when their provider configuration is present. Account linking, delegated wallet authority, durable withdrawal history, Graph-key rotation, publication, and native Hedera x402 signing are not implemented. See [backend/framework.md](backend/framework.md) for the exact implemented and reserved boundary.
