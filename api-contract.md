...
Draft 0.6, updated on 2026-09-07 for human review. This is the target HTTP contract, not a claim that every durable endpoint is implemented. The frontend currently uses the explicitly temporary backend demo runtime documented in [demo-runtime.md](docs/api/demo-runtime.md); it does not use browser business-data fixtures. That runtime now includes a redacted, process-memory Model Service profile used only by an explicit connection test or Agent-plan action. Testing does not save submitted profile values. Privy creator authentication, provider-identity resolution to a stable Sprue user UUID, and idempotent local account/workspace bootstrap are implemented when the required provider configuration is present. Account linking is not implemented. The database and API/standby-worker framework exist; see [backend/framework.md](backend/framework.md) for the exact implemented and reserved boundary.

Inputs: [data model 1.6](data-model.md), [product design 1.36](product-design.md), [current frontend ownership](frontend/README.md), and [implementation status](frontend/implementation-status.md). Database invariants take precedence over fixture behavior. M1-M3 persistence/lifecycle directions were approved on 2026-09-05 and remain in data-model 1.6 and the database foundation. Process probes, public app configuration, configured Privy token verification, provider-identity resolution to a stable Sprue user UUID, creator identity reads, transactional account/workspace bootstrap, and the explicitly temporary backend demo projection are implemented; other durable domain routes remain reserved and return 503 without side effects. The hackathon publication profile charges no Sprue service fee. The demo projection and optional model call do not establish durable provider capability or authorize a Graph request, funded operation, or deployment.
...
M1-M3 directions below were approved on 2026-09-05 and remain incorporated in data-model 1.6. M1 maps to control_commands/command_dispatches; M2 maps to request recovery fields and api_payment_proofs; M3 maps to clarified lifecycle and transaction boundaries. SQL structure and representative guards exist; HTTP semantics and services still need implementation. M4 records the approved model-profile requirement, while M5 records the approved outbound-balance UX; both durable persistence shapes still need human review. E1/E2 remain unresolved. Process memory is permitted only for the explicitly labeled evaluator model profile; do not substitute it for a durable authenticated implementation.
...
| M4 | Creator-selected model URL, model identity, secret rotation, and the exact profile revision used by a planning call have no approved durable resource | Review the proposed `agent_model_profiles` extension in data-model 1.6, store only a secret-manager reference, and pin the profile/secret revision plus credential fingerprint on every model planning call | Model Service, Agent planning, audit, rotation/revocation, and usage controls |
| M5 | Creator-initiated transfer of Graph-funding USDC or x402-revenue HBAR has no reviewed command, transfer-intent kind, fee quote, consent proof, or reconciliation lifecycle | Keep the final transfer submission disabled until the data model and provider-specific commands bind one wallet address, network, asset, destination, amount, fee quote, creator authorization, idempotency key, provider attempt, and reconciled settlement | Wallet balances, outbound transfer dialog, ledger, Privy/Base adapter, Hedera adapter |
