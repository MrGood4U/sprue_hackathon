...
Draft 1.67, updated on 2026-09-12. Builder now treats a semantically changed or repositioned workflow as unsaved until a durable save succeeds. Any internal navigation away from Builder, including the product back arrow, Wallet and Access, Model Service, and product tabs, opens a focused confirmation dialog with `Stay` and `Leave without saving`; browser refresh/close also uses the platform unsaved-change warning. `Save draft` sends the exact current layout-free structured DAG plus a separate node-position layout to an owner-authorized backend record with optimistic concurrency, disables while saving, retains the canvas on failure, and marks the editor clean only after PostgreSQL success. The saved working copy remains separate from immutable ready versions and all deployment/publication pointers. `Run backend build` validates and persists or reuses an immutable ready version, then opens API, but does not activate it. An existing API and x402 gate continue serving their prior active version until the creator explicitly confirms Deploy or Redeploy on API; draft save never creates, updates, suspends, or retires either service.

Draft 1.66, updated on 2026-09-12. Agent stages 2–9 now use progressive disclosure in the center conversation cards. Their collapsed state keeps the stage, status, and exact trace summary compact; one click expands at most one card at a time. Expanded content is persisted, server-allowlisted harness evidence: interpreted output shape and bounded search terms, compiler-owned source requirements, bounded Graph candidates and activity, formal aggregation rankings and validated use/fallback decisions, raw-entity rankings and selections, and per-requirement field alternatives. Embedding-backed entity and field rows may show their actual cosine similarity as an advisory rank value; deterministic rows are labeled as such and never receive a fabricated score. The right progress rail remains compact. Raw vectors, prompts, provider envelopes, credentials, hidden reasoning, and browser-inferred facts are never included.

Draft 1.65, updated on 2026-09-12. The Wallet and Access balance cards place an independent copy control immediately after the complete Base Sepolia funding address and the canonical Hedera account ID. Each control owns its own visible and assistive success/failure state. A `Settings` action next to the Base Sepolia funding and transfer actions opens a focused Privy wallet dialog where the creator must enter a positive USDC daily limit for Sprue's automated Graph x402 purchases, calculated by UTC calendar day. The dialog keeps observed Privy signing authorization separate from the Sprue daily budget, returns focus to its trigger, and cannot be dismissed during save. This limit does not apply to creator-confirmed transfers. Saving records the provider signer reference as pending/unverified and the local budget as draft; live delegated Graph purchasing, signer-secret binding, and provider-policy enforcement remain separate integration gates.
...
Draft 1.61, updated on 2026-09-09. The user promoted the existing frontend to the maintained Sprue product and authorized continued implementation. D1, D2, and D4 are approved, D3 targets large-screen browsers, and the Evidence-First Console direction is selected. The authenticated Product Dashboard now reads workspace-scoped product summaries and 24-hour overview metrics from PostgreSQL through live backend APIs. Its create, rename, and delete actions are durable, idempotent, and concurrency protected. Each product row exposes a garbage-can action that opens a named confirmation dialog before the backend tombstones the product. Creating a product now immediately creates a blank draft and opens its Agent Planner without an intermediate intent modal. The Agent Planner starts with a localized natural-language example in the intent placeholder, reads the durable conversation, invokes the saved model and selected Graph credential on explicit generation, and renders persisted source evidence and trace summaries without a demo fallback. Every real harness stage also appears as its own conversation card: an active stage shows a rotating indicator, an explicit in-progress label, elapsed request time, and its persisted start summary; a terminal stage shows its persisted completion or failure summary. These cards expose reviewable trace facts, never hidden model reasoning. Source exploration now exposes a separate entity-selection stage after Graph discovery: the model selects supplied query-entity references from compact summaries, the Harness expands only those validated entities' fields, and a later model stage binds exact fields and proposes the DAG. The active conversation surface shows only the newest planning run; confirming `Recreate plan` removes the previous run's visible intent, stage, and result cards immediately, while the backend retains the older messages and traces for auditability. External model and Graph steps may run for several minutes, with a long whole-run deadline and browser wait that preserve the visible trace instead of failing at the former 125-second cutoff. Before the first run, the composer offers both Agent generation and a secondary `Create manually` action that opens Build without invoking the Agent. A failed, cancelled, clarification, or unsupported terminal result also retains this manual escape route; it disappears only while planning is active or when a compilation-ready proposal instead offers `Review DAG`. During a real run, those non-running actions are replaced by the disabled activity action and an explicit, confirmed `Stop run` action backed by cooperative server cancellation; the stop action disappears after the command reaches any terminal state. Agent, Build, API, and Monetize tabs now preserve the same live product reference and remain freely navigable in every planning state. Their shared workspace-scoped product cache keeps the resolved name stable across tab switches; cached Dashboard summaries are presentation-only and cannot bypass a detail page's loading state or initialize fields absent from that summary. A fresh deep link reserves title space with an accessible skeleton instead of exposing the internal product ID. A creator-route render failure is contained by a localized recovery state with Retry and Back to products actions rather than blanking the application. Builder reads the newest durable Agent run and renders its validated source/entity/field/operator handoff as an editable, explicitly non-executable planning draft; failed or absent plans open an empty draft instead of sample data. Browser-session edits survive tab navigation but are not durable versions, and backend Build remains unavailable until source admission and executable compilation succeed. API and Monetize now read one owner-authorized backend projection of the real product version, deployment, active materialization, x402 publication, Hedera recipient capability, confirmed revenue, and paid request facts. Missing facts render explicit unavailable states; these pages never substitute demo success. Deployment, private-test, publication, and payment mutations remain unavailable until their reviewed services exist. The public product remains on the separately identified evaluator runtime. The wallet behavior and Graph credential lifecycle approved through Draft 1.46 remain unchanged.

The 2026-09-09 long-running-step amendment keeps the small functional Agent spinners visibly active under a reduced-motion operating-system preference by using a slower stepped rotation, while elapsed-time text remains the primary non-motion signal. Each external model or Graph step defaults to a 600-second bound, the whole run defaults to 3,600 seconds, and the browser planning transport remains open beyond the backend's maximum configurable run bound while the view is mounted. No automatic paid retry is added.
...
The account control sits at the far right of shared application and product headers, after the language selector, and both controls are aligned to the header's top edge. Page-local primary actions stay with the content they affect instead of displacing these global controls. Its compact avatar button opens a menu anchored below the trigger containing the authenticated identity, current workspace, links to the existing Product Dashboard, Wallet & Access, and Model Service routes, followed by a visually separated sign-out action. It does not invent Team, Billing, or account-preference routes that are outside the current page inventory. The button exposes its expanded state, the menu is keyboard reachable, and identity or workspace text remains selectable without dismissing the menu. `Escape` returns focus to the trigger; a true outside pointer interaction, another avatar activation, destination selection, or sign-out dismisses it. Focus transitions to another control outside the menu also dismiss it, while a null focus destination caused by interacting with non-focusable content inside the menu does not. Environment and provider readiness are not persistent navigation content; they belong in the Wallet & Access page or the relevant product workflow. Destructive actions do not belong in primary navigation and sign-out remains separated inside the account menu.

Inside a product, a persistent product header shows the product name and four deep-linkable views. The authenticated application shell keeps the last resolved product identity in a workspace-scoped memory cache, refreshed by Dashboard/detail reads, creation, and renaming, so switching among Agent, Build, API, and Monetize never flashes the internal product ID while each destination performs its normal live read. A fresh deep link without cached identity reserves the title dimensions with an accessible skeleton; it does not render the route ID as user-facing copy. The product name and adjacent pencil action both enter inline editing. The controlled field receives visible focus; `Enter` or focus leaving the field commits a non-empty trimmed name, while `Escape` restores the prior value. An empty field restores the prior value instead of erasing the product name. Version, deployment, and workflow status are shown within the page where they support a decision rather than as always-visible header controls:
...
1. **Privy account wallet**
   ...
   - Complete Base/Base Sepolia address for Graph spending, allowed to wrap without abbreviation, plus an adjacent copy action with accessible success or failure feedback. The canonical Hedera account ID in its separate balance card follows the same adjacent-control pattern and owns an independent feedback state.
   ...
   - A `Settings` action in the Base Sepolia balance card opens a focused dialog that shows additional-signer observation and the current Sprue daily budget as separate statuses. The creator enters a required positive USDC limit for one UTC calendar day; saving a new grant first uses Privy's visible authorization flow, then the backend records the resulting signer/policy reference as pending verification and persists the local budget as draft.
   - The daily limit applies only to Sprue's automated Graph x402 purchases. Creator-confirmed withdrawals stay under the wallet owner's direct control. The setting is not evidence that the unfinished delegated Graph purchase adapter has enforced or spent the budget.
...
- Semantic entity retrieval with current embedding-batch, local similarity-calculation, and retained-candidate summaries.
- Candidate/query-entity selection from compact inspected summaries.
- Source and operator feasibility assessment.
- Deterministic evidence and graph-shape validation.

Each step exposes a localized status and a concise evidence description. Step completion and percentages are driven only by backend trace events. In addition to the compact right-side timeline, each observed harness stage is rendered as one stable conversation card. A card updates in place from `started` to `passed` or `failed` and shows the exact sanitized backend trace summary. Repeated `started` events for semantic entity retrieval update that same card with the current source-need and embedding-batch position, then identify the local cosine-similarity calculation; the terminal event reports embedded and retained counts. Similarity calculation remains a substate because it is local and normally millisecond-scale, while the externally bounded embedding operation remains independently visible and can fail with specific recovery guidance. The active conversation surface renders only the newest run, beginning with its user intent and followed by that run's stage and terminal result cards. Starting a replacement run clears the earlier run from this surface immediately; older messages and traces remain durable server-side evidence and are not deleted. While the explicit synchronous planning request is genuinely in flight, the active card and disabled action use rotating indeterminate indicators, and the page shows elapsed wall-clock time measured from that browser request's start. This elapsed label is activity feedback, not durable backend trace evidence; it resets when the request ends. One atomic conversation live region announces changes; cards also use icon plus text so status is never encoded by motion or color alone. Reduced-motion preferences may slow the decorative rotation because the text status and elapsed duration remain visible. No synthetic stage timer or inferred result may substitute for a backend trace event.

Stages 2–9 add a click-expanded evidence body to these cards while preserving the concise default view and compact right-side timeline. Only one evidence body is open at a time. Lists are bounded before persistence and grouped by source need or semantic requirement to prevent a single card from becoming an undifferentiated table. The surface distinguishes provider discovery fit, embedding cosine similarity, and deterministic ranking; it does not turn cosine similarity into a confidence percentage. A stage with no applicable aggregate or fallback path shows that real empty result rather than remaining visually pending or inventing candidate data.

**Transitions:**

- Before the first plan run, the composer offers `Create manually` and the primary `Generate plan` action. `Create manually` opens the product Build route immediately and does not create an Agent session, submit a message, or call the model or Graph adapter.
- During the current synchronous implementation, the composer replaces both pre-run actions with a disabled `Generating plan` action and a destructive `Stop run` action. `Stop run` names the consequence in a confirmation dialog, requests cancellation for the exact active command, remains disabled while cancellation is being accepted, and does not navigate away. The command cooperatively aborts the active model or Graph operation through the same run signal, persists a terminal cancelled result, and retains completed trace evidence. The browser keeps rotating activity indicators and `Agent running for …` elapsed feedback, including hours, until the server reports a terminal command. The browser does not end a healthy long-running request at the former 125-second cutoff.
- After a persisted result exists, the composer offers `Recreate plan`. Confirming it replaces the active run view immediately, so previous intent, step, and result cards cannot be mistaken for part of the new run. Failed, cancelled, clarification, and unsupported results also retain `Create manually` as a secondary recovery route into an empty or feasibility-derived Builder draft. `Review generated DAG` replaces that manual action only when the backend marks the proposal compilation-ready; source-feasibility results with admission blockers remain on Agent unless the creator deliberately chooses the manual route.
- Product tabs remain enabled before, during, and after planning, and always preserve the current product reference. Opening Build before readiness shows an empty or feasibility-derived non-executable planning draft; the dedicated `Review generated DAG` action still appears only for a compilation-ready plan.
...
`Run backend build` submits the current structured DAG without canvas coordinates or other presentation state. The button remains disabled with a rotating status icon until the authenticated preflight returns. A successful result moves directly to the product's API page. A semantic failure leaves every edit in place and opens one concise dialog with the compiler's bounded issue code, affected node when available, and human-readable explanation; request failures use the same recovery surface. Closing the dialog returns to Builder so the creator can correct the graph and retry. This interaction does not imply that a source was admitted, a version was persisted, GraphQL was generated, data was executed, or an API was deployed.
...
**Domain reads and writes:** The implemented slice reads the workspace product plus its newest durable `agent_sessions` and `agent_messages` and projects the proposal's backend-owned `builderDraft`. Edits are stored only in browser session state keyed by workspace, product, and proposal identity; a newer Agent result invalidates them. It does not yet write `data_product_versions`, source snapshots, layout, or execution records. The target durable boundary includes `source_snapshots`, `data_product_versions`, `data_product_version_sources`, `product_version_layouts`, `execution_runs`, `run_attempts`, `node_runs`, `source_requests`, `source_http_attempts`, `artifacts`, `materializations`, `trace_streams`, `trace_events`, `budget_reservations`, and upstream payment records.
...
6. **Review:** network, asset, amount, recipient, protocol version, scheme, timeout, facilitator, and public route. The hackathon profile has no Sprue service fee.
...
- Separate HBAR cards for gross sales, creator proceeds, and confirmed provider/network fees.
- The hackathon profile has no Sprue service fee, fee allocation, or fee configuration surface; the configured buyer price flows entirely to the creator before independently evidenced provider or network costs.
...
- Funding, Graph expenses, gross sales, creator proceeds, and provider/network fees remain distinct categories. Future-compatible platform-fee fields stay disabled and zero in the hackathon profile.
...
The maintained product frontend is in [`frontend/`](frontend/). It includes all ten page families, shared components, locale catalogs, and feature hooks. Dashboard, Wallet and Access, Model Service, Agent Planner, and the Builder planning-draft read use live workspace APIs. API, Monetize, and the public product use a server-generated demo runtime while their durable backend integration remains unfinished. Continue implementing this source directly. [Frontend implementation status](frontend/implementation-status.md) records the remaining behavior and service gaps.
...
| 2026-09-07 | Recorded Draft 1.33 with a consistent grab cursor across the complete Builder interaction layer while hand mode is active | Make the selected tool visually truthful over empty canvas, nodes, ports, and edges without changing overlay-control cursors |
| 2026-09-07 | Recorded Draft 1.34 with a complete copyable wallet address, separate Base USDC and Hedera HBAR balances, and balance-scoped transfer entry points | Keep unlike funds and networks distinct, colocate each action with the balance it affects, and fail closed until reviewed signing and transfer commands exist |
...
| 2026-09-09 | Recorded Draft 1.52 with one stable conversation result card per observed real harness stage, backed by an owner-authorized cursor-polled active trace and the persisted terminal message trace | Make every step and its actual result visible, including an explicit running state, without fabricating model reasoning or browser-owned stage completion |
| 2026-09-09 | Recorded Draft 1.53 with long-running Agent steps: 600-second default external-step bounds, a 3,600-second default whole-run deadline, and a browser wait longer than the maximum configurable run | Keep legitimate model and Graph work alive while real trace cards continue to communicate progress instead of producing a premature browser failure |
| 2026-09-09 | Recorded Draft 1.54 with mutually exclusive pre-run manual creation and in-flight cooperative cancellation controls | Let creators bypass Agent planning before it starts or explicitly stop the exact active command without pretending that aborting a browser request cancels server work |
| 2026-09-09 | Recorded Draft 1.55 with a latest-run-only Agent conversation surface while retaining earlier messages and traces as durable backend evidence | Prevent stale step cards from being read as part of a replacement run without destroying the audit trail |
| 2026-09-09 | Recorded Draft 1.56 with a distinct compact entity-selection trace stage followed by Harness-owned selected-field expansion | Let the model choose the best inspected entity without asking it to reproduce every field, while keeping field evidence and progress deterministic |
| 2026-09-09 | Recorded Draft 1.57 with invariant four-tab product navigation and a live editable Builder projection from the newest durable Agent result | Keep product identity stable across views and remove the Builder's sample DAG without overstating feasibility as an executable version |
| 2026-09-09 | Recorded Draft 1.61 with presentation-only cached product summaries and a recoverable creator-route render boundary | Prevent cached Dashboard rows from bypassing detail readiness and ensure an unexpected render fault never leaves a blank Creator Console |
| 2026-09-10 | Recorded Draft 1.62 with a dedicated semantic entity-retrieval trace card and real embedding-batch, similarity, and retained-count summaries | Keep a potentially multi-request provider operation independently observable without promoting millisecond-scale local cosine calculation into a misleading standalone step |
| 2026-09-11 | Recorded Draft 1.63 and connected Builder to a read-only backend compilation preflight | Make the current structured DAG fail or advance on authoritative graph, operator, and schema checks instead of a browser-only readiness impression; keep source admission, durable versions, data execution, and deployment explicit follow-up work |
| 2026-09-12 | Recorded Draft 1.66 with click-expanded, bounded, persisted evidence for Agent stages 2–9 | Expose actual semantic requirements, rankings, aggregate decisions, selections, and field alternatives without cluttering the progress rail or fabricating scores and hidden reasoning |
