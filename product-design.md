...
- During the current synchronous implementation, the composer shows one disabled `Generating plan` action, rotating activity indicators, and `Agent running for …` elapsed feedback, including hours for long runs. The browser does not end a healthy long-running request at the former 125-second cutoff. It does not advertise cancellation that the server cannot perform.
