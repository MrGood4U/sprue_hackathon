...
Each step exposes a localized status and a concise evidence description. Step completion and percentages are driven only by backend trace events. In addition to the compact right-side timeline, each observed harness stage is rendered as one stable conversation card. A card updates in place from `started` to `passed` or `failed`, shows the exact sanitized backend trace summary, and remains in the durable conversation after the terminal Agent reply. While the explicit synchronous planning request is genuinely in flight, the active card and disabled action use rotating indeterminate indicators, and the page shows elapsed wall-clock time measured from that browser request's start. This elapsed label is activity feedback, not durable backend trace evidence; it resets when the request ends. One atomic conversation live region announces changes; cards also use icon plus text so status is never encoded by motion or color alone. Reduced-motion preferences may slow the decorative rotation because the text status and elapsed duration remain visible. No synthetic stage timer or inferred result may substitute for a backend trace event.
