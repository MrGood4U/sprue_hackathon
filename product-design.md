...
- After a persisted result exists, the composer offers `Recreate plan`. Confirming it replaces the active run view immediately, so previous intent, step, and result cards cannot be mistaken for part of the new run. `Review generated DAG` appears only when the backend marks the proposal compilation-ready; source-feasibility results with admission blockers remain on Agent.
