...
This document records the completed project-structure conception and selected technical/deployment boundaries. [Data-model version 1.8](data-model.md) is the approved implementation baseline, with provider-independent Sprue user IDs, workspace tenant isolation, durable encrypted Model Service profiles, and Hedera testnet HBAR through Blocky402 as the initial downstream profile. The ten-page application under `frontend/` is the maintained product frontend, with public Entry and Creator Login separated. The database foundation, API/standby-worker scaffolding, Privy creator account bootstrap, and Model Service persistence are implemented (see [database.md](backend/database.md)); the temporary evaluator runtime may resolve that profile for an explicit Agent-plan action. Durable product/provider integration remains pending, and [api-contract.md](api-contract.md) Draft 0.7 defines the HTTP boundary. Frontend/backend ownership, the Evidence-First Console direction, and the browser-only product boundary remain in force.
...
- Exact `@x402` package versions; Hedera testnet, HBAR, x402 v2 `exact`, and Blocky402 are selected for the initial downstream integration.
