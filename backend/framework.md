...
- Every durable business route is reserved in a domain-specific route catalog. Reserved routes return `503 CAPABILITY_NOT_IMPLEMENTED` after applicable transport/authentication/owner checks, never synthetic DTOs, 202 acceptance or 402 payment challenges. The explicitly temporary evaluator routes under `/api/v1/public/demo/` are an exception: when `DEMO_RUNTIME_ENABLED=true`, they return a server-generated fixture-backed projection and may call only the session-configured OpenAI-compatible model during an explicit Agent-plan action. They perform no durable mutation, Graph request, wallet action or payment. Resource ancestry checks must be added before enabling each durable handler; a route reservation is not an implemented API.
