...
| `migrations/0022_live_deployments.sql` through `0024_live_paid_api.sql` | Live immutable versions, API activation, credentials, x402 publication and paid-delivery evidence |
| `migrations/0025_builder_drafts.sql` | Mutable Builder DAG and separate node layout, isolated from immutable versions and active deployment pointers |
...
Tests always create isolated, in-memory PGlite databases and ignore `DATABASE_URL`; they never migrate an existing database. They verify empty initialization, repeated migration/seed, checksum drift rejection, failed-migration rollback, all 55 tables/741 fields, provider-binding resolution, multiple identities for one stable user, cross-workspace actor rejection, encrypted model-profile and Graph-key persistence/restart/isolation/tamper rejection, durable isolated Builder drafts, idempotent Privy wallet binding, real-balance adapter projection, and representative negative transaction/retry/ownership/money/provenance cases. Fixture DAGs test relational structure only and are deliberately not executable H1 schemas.
