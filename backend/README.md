...
See [api-contract.md](../api-contract.md) Draft 0.19 and its domain documents under `docs/api/` for route, request/response, authorization, idempotency, concurrency, trace, model-profile, wallet, credential, and payment boundaries. M1-M4 directions remain mapped into data-model 1.13. Creator authentication, provider-identity resolution, account/workspace bootstrap, owner-authorized demo surfaces, durable encrypted model-profile and Graph-credential lifecycle operations, user-wallet provisioning/binding, current Privy/Hedera balance reads, explicit Hedera testnet activation, complete-account control projection, direct creator-confirmed testnet withdrawals, and the read-only product delivery projection are implemented. Account linking, most mutation handlers, durable withdrawal history/reconciliation, delegated wallet authority, Graph-key rotation, source-specific compatibility validation, and paid publication services remain unavailable. Provider and capped-buyer gates remain prerequisites for delegated or paid financial behavior.
