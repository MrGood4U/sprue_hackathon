...
| P1. Plan source search | Sanitized intent, versioned Subgraph-capable network catalog, and relevant conversation decisions | Model emits dynamic source requirements, protocol/network-scoped asset semantics, and result fields without a fixed entity/grain/metric vocabulary, plus minimal keywords for each requirement; controller validates them and derives at most three network-specific searches per need | SourceDiscoveryPlan v3, or a small clarification; no model-selected MCP tool, URL, source ID, query, or DAG |
...
| P3. Select source entities | SemanticPlan plus compact candidate and entity summaries without full field arrays | Second model pass selects exactly one supplied candidate/query entity per source need; controller validates every reference and suitability state | SourceEntitySelection v1, clarification, or unsupported; no field binding or operator proposal |
| P3a. Expand and assess feasibility | Validated entity selection, trusted discovery snapshot, source roles, versioned operator registry, and limits | Harness expands only selected entities; third model pass binds required semantics to exact inspected paths and proposes a generic primitive DAG; deterministic code validates references, types/nullability, expression ASTs, configs, ports, connectivity, output promises, cycles, and limits. Requested ordering must be an explicit Sort / Top K immediately before the single-input Output node. | Non-executable SourceFeasibilityPlan v2, clarification, or unsupported; unknown live checks stay explicit |
...
Implemented exploration shape, schemaVersion 3 (illustrative names only):
...
`SourceDiscoveryPlan` nests this document as `semanticPlan` and includes `searches: [{sourceNeedId, keywords}]`. There is exactly one search entry per source requirement and no more than three semantic hints for each requirement. `protocol` is nullable and `assets` may be empty, so no protocol, asset vocabulary, entity, field, grouping, or output shape is hard-coded. For a market request, every asset is scoped by the parent `dataNetwork`; symbols guide discovery while an optional `networkAssetId` records only an identity explicitly supplied by the creator. The controller deterministically adds the server-owned network label and canonicalizes asset-pair ordering before calling the bounded Graph search. Descriptions become executable only after P3 binds exact inspected fields and lowers calculations into the typed operator AST.
...
The source adapter boundary is intentionally narrower than a general-purpose MCP client. The first model proposes semantic field requirements, optional protocol/version, network-scoped assets, and keyword strings for independent SourceNeed records. The controller validates them, combines them with the server-owned network label into at most three network-specific search phrases, and maps each SourceNeed to the fixed keyword-search or creator-supplied contract lookup path; the model never sees or chooses the MCP tool name. Following the official Subgraph MCP order, the controller obtains 30-day activity for every bounded potentially relevant manifest CID before source selection, orders candidates from highest to lowest observed use with missing observations last, then inspects every candidate schema within each SourceNeed's independent budget. Zero activity remains visible evidence but is not a schema-inspection or suitability veto. When provider source SDL omits the generated `Query` root, the adapter uses one fixed introspection document through the identifier-bound Graph operation to obtain runtime query fields; it does not guess pluralized entry points. The adapter extracts actual query entities and field paths with Graph type, inferred semantic value type, list shape, and nullability. Lexical matches against model hints are ranking suggestions only; they do not prove semantics and cannot create a field. The entity-selection model chooses only supplied candidate/query-entity references from compact summaries. The Harness validates those references, expands fields only for the selected entities from trusted discovery evidence, and the post-selection model binds exact returned paths. Deterministic code checks every binding and its type/nullability. The adapter exposes only reviewed metadata capabilities and accepts neither a model-selected MCP server, URL nor arbitrary tool name. Query-execution tools are absent from its public planning interface. `query.compile` remains a Sprue-owned compiler. A later runtime `executeGraphQL` adapter is invoked only by an authorized data-plane operation with a stored query plan and access context.
