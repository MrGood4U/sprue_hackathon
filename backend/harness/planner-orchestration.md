...
Draft 0.6, 2026-09-10. The first cross-chain swap slice of this controller is implemented under `backend/src/modules/agent/harness/`. The live-source exploration order is now explicit: a model first emits a strict semantic plan and bounded Subgraph search keywords, the controller invokes restricted Graph MCP metadata operations, an optional no-cache embedding pass ranks bounded inspected entity schemas, a second model pass selects query entities from compact summaries, and the harness expands only those selected entities before a third model pass binds fields and proposes a registered-operator composition. That last pass separates first-pass semantic requirement bindings from purpose-tagged auxiliary bindings discovered in the inspected Schema. Deterministic checks validate every artifact and return a non-executable feasibility result. The earlier three-pass path remains as a fixture compilation compatibility path for already inspected sources. Durable source admission, live Graph execution, checkpoints, and general H1/H3 coverage remain pending. The fixture-backed evaluator is not live provider evidence.
...
| O4a Retrieve entities | Inspected entity schemas and source needs | Optionally issue bounded embedding batches, calculate cosine similarity locally, preserve candidate diversity, and emit real batch/substate trace progress | Compact ranked entity evidence; no model or Graph data query |
...
Input is the validated `SemanticPlan`, compiler-owned `SourceNeed[]`, and bounded candidate/entity summaries from schema inspection. When configured, Sprue first embeds a bounded semantic source-need document and bounded documents containing actual inspected entity and field names. It computes cosine similarity locally, uses those scores to order the compact evidence while retaining a fair share across candidate Subgraphs, and sends the selector only references, counts, advisory matches, and similarity scores—not full field arrays or vectors. The embedding pass has no cache and no selection authority; failed configured retrieval is explicit. Output is `source_entity_selection`, `clarification`, or `unsupported`. A selection must copy exactly one supplied `candidateRef` and inspected `queryEntity` for every source need. It cannot bind fields, compose operators, or invent a source, entity, field, tool, URL, access mode, credential, payment, or GraphQL document. The harness validates every reference and expands the selected entities from its trusted discovery snapshot.
