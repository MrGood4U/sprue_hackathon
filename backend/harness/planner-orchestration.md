...
Draft 0.7, 2026-09-11. The live-source controller now performs aggregate-first selection per source need. The first model pass may declare both a raw row contract and a compatible pre-aggregated alternative. Restricted Graph metadata discovery preserves formal timeseries and aggregation directives. The Harness ranks only formally declared aggregation entities first, asks a narrow model pass whether each candidate satisfies the pre-aggregated contract, and deterministically validates the chosen interval, dimensions, measures, aggregate functions, and field types. Only uncovered source needs proceed to raw entity embedding and compact selection. The Harness merges both paths before field retrieval and final feasibility composition. A provider-to-semantic project Map remains explicit for aggregate and raw sources alike.
...
This design does not approve the open H1 executable schemas or H3 live-source and operating profile. The implemented path can make model calls and Graph metadata calls only when the application injects the corresponding configured ports. It does not authorize a Graph data query, product version, payment, deployment, or runtime execution.
...
2. Entity selection: after metadata discovery and schema inspection, optionally embed bounded actual entity/field documents, then choose exactly one returned candidate and query entity per source need from compact ranked evidence without full field arrays.
...
The controller can return four visible outcomes across exploration and compilation:

- `feasibility`: inspected existing-source choices and a deterministically valid primitive topology, explicitly not ready for compilation or execution;
...
| Entity-schema retriever | Embedding a bounded source-need description and actual inspected entity/field documents, computing cosine similarity, and preserving candidate diversity | Selecting sources, weakening hard gates, querying live rows, caching vectors, or granting provider authority |
| Entity-selection model pass | Comparing compact inspected candidate/entity summaries and advisory retrieval scores, then selecting one supplied entity per source need | Binding fields, composing operators, inventing candidates/entities, querying live rows |
| Post-selection feasibility model pass | Binding exact required fields, declaring purpose-tagged auxiliary fields from harness-expanded selected entities, and proposing necessary registered operators with typed configuration | Changing the selected source/entity, inventing paths/operators, disguising an auxiliary field as a first-pass requirement, bypassing schema checks, querying live rows, claiming its graph is valid |
...
| O1 Plan discovery | Sanitized intent, versioned Subgraph-capable network catalog, prior decisions, parent semantic summary | Run the first model pass; validate a dynamic `SemanticPlan`, source requirements, arbitrary typed fields/results, and one bounded keyword set per requirement | `SourceDiscoveryPlan`, `clarification`, or `unsupported` |
...
| O4a Retrieve aggregates | Formal `@aggregation` entity metadata and aggregate-capable source needs | Optionally issue bounded embedding batches over aggregate schemas, calculate similarity locally, and retain candidate diversity | Compact ranked formal aggregation evidence; no model or Graph data query |
| O4b Assess aggregate fit | Ranked aggregation evidence and pre-aggregated contracts | Ask the model to choose exact candidate/entity/interval/field bindings or explicit fallback, then validate all directive facts deterministically | Per-need aggregate selections and fallback reasons |
| O5 Select raw entities | Compact non-aggregation candidate/entity summaries for uncovered source needs | Run raw embedding and entity selection only for uncovered needs; validate every returned reference | Raw fallback selections or source gap |
...
The normal source-exploration path uses three calls when no formal aggregate is viable or when aggregates cover every eligible need, and four calls for a mixed aggregate/raw result. At most one repair is shared inside the five-call limit. A retry resumes from the latest valid checkpoint and cannot reset counters by changing IDs or wording.
...
The live exploration `SemanticPlan` records what the product means before any source is selected using schemaVersion 2. It contains dynamically named `sourceRequirements[].fields` and `result.fields`; it does not prescribe an entity, event kind, metric, grain, grouping key, or output shape. See the complete example in [workflow](workflow.md#p1-sourcediscoveryplan-and-semanticplan).

The schemaVersion 1 wallet-shaped document below is retained only to describe the isolated fixture compatibility compiler. It is not accepted as the live exploration contract and must not be used to decide whether an arbitrary creator request is supported.
...
The controller copies validated version-2 source requirements into `SourceNeed` records without changing their semantic field vocabulary. One request may need one source per network, multiple entities from one source, or multiple sources on one network. The first model pass performs this semantic decomposition; deterministic code verifies IDs, networks, fields, limits, and one search entry per need.

```json
{
  "id": "metric_events",
  "dataNetwork": "eip155:42161",
  "description": "Raw records required by this branch of the request",
  ...
  "fields": [
    ...
    {"id": "raw_value", "description": "numeric value", "expectedType": "decimal", "unit": null, "required": true, "allowNullable": false, "hints": ["value"]}
  ],
  "constraints": []
}
```

The model cannot collapse distinct required network semantics merely because a candidate is convenient. Conversely, the controller does not force a wallet grain, one need per network, or multiple source nodes when one inspected Subgraph supplies all required fields at the required grain and coverage.
...
The controller-owned planning capabilities remain the typed contracts in [tools.md](tools.md). A model stage may propose schema-bound inputs such as keyword hints or candidate references, but it does not dispatch these capabilities directly:

| Concern | Controller capability | Deterministic implementation supplied by Sprue |
|---|---|---|
...
### Pass A: Discovery planner

Input is the current creator message, an allowlisted network catalog, bounded prior decisions, and an optional parent semantic summary. Output is exactly one of `source_discovery_plan`, `clarification`, or `unsupported`. A discovery plan contains a strict schemaVersion 3 `SemanticPlan`, one to four independent source requirements, optional protocol/version, network-scoped asset symbols or explicitly supplied identities, dynamic field/result definitions, and one bounded semantic keyword set per source requirement. The controller rejects unknown networks, missing or duplicate source/field identifiers, duplicate asset identities, unsupported value types, invalid result references, excessive requirements, and excessive keywords. It then combines protocol, canonical asset-symbol ordering, the server-owned network label, and the model hints into at most three independent searches per need. This pass cannot name a source candidate, MCP tool, server or endpoint, and cannot emit a query or DAG.
...
### Pass B: Post-discovery aggregate selector

Input is only source needs that declare a pre-aggregated alternative and inspected entities carrying formal `@aggregation` metadata. Optional embeddings rank those bounded aggregate documents. Output is `aggregate_selection`, `clarification`, or `unsupported`. Each need must either copy one supplied candidate/entity reference with an exact declared interval and complete dimension/measure bindings or explicitly fall back. The Harness verifies directive provenance, accepted aggregate functions, scalar types, nullability, and interval membership. Direct row/event-count requirements use the provider `count` function even when Sprue later sums those bucket counts across dimensions; additive amount measures use `sum`. Names alone cannot establish aggregate capability.

### Pass C: Raw entity fallback selector

Only source needs not satisfied by Pass B are embedded and compared with non-aggregation entities. A selection must copy exactly one supplied candidate/entity reference per uncovered need. It cannot bind fields, compose operators, or invent a source, entity, field, tool, URL, access mode, credential, payment, or GraphQL document. The Harness validates every reference and merges raw selections with the accepted aggregate selections.

### Pass D: Post-selection feasibility planner

After the entity selector succeeds, the harness embeds every field from each selected entity as a separate bounded document and embeds every declared semantic field requirement as a query. It computes all field-to-requirement cosine similarities locally and builds the union of the highest-ranked alternatives for every requirement; it does not discard fields through a fixed global prefilter before this calculation. Input to the feasibility planner is the validated `SemanticPlan`, compiler-owned `SourceNeed[]` and source roles, this retrieved field view for only the harness-expanded selected entities, registry signatures, and topology limits. The complete selected schemas remain in trusted harness memory. Output is `source_feasibility`, `clarification`, or `unsupported`. A feasible output must copy the locked candidate/entity references, bind every required field to an exact presented and inspected field path, and include a typed `CompositionIntent`. When an inspected provider field is needed only to realize a filter, join, group, sort, derivation, or output projection, the model adds an entry to `auxiliaryFieldBindings` with a new logical name and that purpose instead of inventing a `SourceNeed` requirement ID. It cannot change source selection or invent a field path, tool, operator, expression form, URL, access mode, credential, payment, or GraphQL document. When embedding retrieval is disabled, a bounded deterministic field-ranking view preserves compatibility.

The deterministic gate rejects incompatible candidates, missing or duplicate required-field bindings, auxiliary name/path collisions, list-valued or uninspected auxiliary paths, auxiliary fields unused for their declared operator purpose, invented entity/field paths, incompatible types, unknown roles/operators/ports, invalid typed expressions/configuration, duplicate inputs, disconnected nodes, cycles, output-schema mismatches, and limit violations. A complete stable Filter may be moved into an Agent-authored GraphQL `where` object only when bounded canonicalization proves the full predicate equivalent. Relationship paths additionally require inspected non-list, non-null field evidence, exact relationship-filter nesting, and complete AND/OR branch coverage. The executable Graph Source subset permits only one child-filter relationship hop because the provider rejects nested child-filter joins; deeper, partial, nullable, or list relationship pushdown remains rejected. Passing this gate means only that a schema-evidenced source/operator arrangement appears structurally possible. Before persisting a new immutable live version, the authorized build path also executes each authored Source query with `first: 1` and a resolved runtime window; a provider rejection fails the build without changing any active deployment. This probe does not establish complete historical coverage, freshness, cost, or result correctness.

### Compatibility compilation passes

The evaluator compatibility path for caller-supplied, already inspected `SourceInput` records retains its semantic, source-selection, and DAG-composition passes. It exists to keep the fixture-backed vertical slice working while O5b is implemented. It must not be chained after discovery in a way that repeats model decisions or silently treats MCP metadata as a durable executable source.
