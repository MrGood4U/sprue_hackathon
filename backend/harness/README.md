...
After compact entity selection, the source-feasibility call receives a separate bounded field view instead of the complete selected schema. The view keeps the strongest inspected suggestion for every semantic requirement, row-local fields, fairly interleaved alternative suggestions, and then remaining fields ranked by generic requirement-token and type relevance. Each entity reports its complete and omitted field counts, while both the retained field JSON and the number of fields are bounded. The complete inspected entity remains inside the Harness: a model may bind only a path shown in its compact view, and the Harness validates that path and type again against the complete schema before accepting the composition. This bounds model context without hardcoding a protocol, asset, network, query entity, or provider field name.
