...
Aggregate cardinalities retain structural count provenance independently from provider measurement units. A semantic result may therefore name the counted domain without a protocol-specific unit vocabulary, while non-count measurements still require exact unit preservation. A `safe_divide` whose denominator is a cardinality proven nonzero for every emitted grouped row preserves the numerator unit and non-nullability; otherwise it remains nullable. These rules depend on validated operators and row shape, not output-field names, protocols, assets, networks, or provider fields.

Lexical field and row-grain vocabularies are advisory ranking aids only. Identifier matching also supports ordered semantic tokens, so provider modifiers can appear between requested terms (for example, a requested `amountUSD` may surface as `amountInUSD` or `amountOutUSD`) without a request-specific field allowlist. The entity-selection model receives compact summaries without full field arrays. Those summaries are allocated fairly across ranked candidates rather than allowing one broad schema to consume the per-need budget. After deterministic reference validation, the harness expands only the selected entities from trusted discovery evidence for the feasibility model. That model may select exact inspected paths that match no vocabulary entry; deterministic validation checks the selected path, scalar type, list shape, and nullability rather than requiring a lexical hit. Fields needed only after inspecting the selected entity—for example a provider-specific filter, join key, grouping key, or sort key—use explicit auxiliary bindings instead of invented first-pass requirement IDs. The harness types those bindings from trusted Schema evidence and requires every auxiliary binding to be consumed by an operator matching its declared purpose.

Draft 2.0, 2026-09-11. The provider-neutral staged harness is implemented under `backend/src/modules/agent/harness/`. It can call the creator's workspace-scoped OpenAI-compatible model profile; the mock model is a deterministic test fixture. The live source-exploration path is schema-driven. The first model response decomposes arbitrary requested semantics into independent source requirements with dynamic protocol, network-scoped asset, field, grain, constraint, result contracts, optional pre-aggregated alternatives, and bounded semantic search hints. The controller builds an independent broad-recall search ladder per source need: the primary protocol/topic by itself, followed by that topic combined with human network aliases from the server-owned catalog. It removes environment suffixes such as `Mainnet` and `Testnet` from search aliases because provider catalog names frequently omit them. Asset pairs remain intact in the semantic SourceNeed but are not forced into Subgraph-name searches; inspected entities, fields, network evidence, and later source admission establish whether a protocol-wide source can serve the requested pair. A concrete network asset identifier must be creator-supplied or later bound from inspected evidence. For each requirement, the restricted Graph adapter collects all bounded candidates' 30-day activity, orders schema inspection by observed use, and inspects every ranked schema within that requirement's budget even when activity is zero or missing. It reads explicit `Query` roots when present. When provider source SDL omits that generated root, the controller uses one fixed introspection document through an identifier-bound Graph adapter operation to read the runtime Query fields; it never guesses a pluralized query name. The normalized query-root/entity/field projection, including formal timeseries and aggregation directives, is cached in Redis by immutable manifest IPFS CID and schema hash, without workspace identity, credentials, user text, provider payloads, or request-specific field bindings. For each aggregate-capable need, the Harness first ranks only entities formally declared with `@aggregation`, and a narrow model pass proposes use or fallback. Deterministic validation requires an exact declared interval plus compatible dimension and `@aggregate` measure bindings. Raw entity embedding and compact selection then run only for uncovered source needs. The final model response binds exact fields for the merged aggregate/raw selections and proposes the registered-operator composition. Deterministic code verifies candidate/entity/path references, aggregate directives, required-versus-auxiliary namespaces, scalar/list shape, type and nullability compatibility, expression ASTs, output fields, ports, connectivity, acyclicity, and limits. Source contracts retain inspected provider paths; every Source still connects exclusively to an explicit project-mode Map, so provider normalization is never hidden in Source. Graph discovery uses a checked-in projection of the official The Graph Networks Registry and exposes every registry entry that advertises the Subgraphs service. This exploration result remains untrusted until every deterministic gate passes.
...
source exploration
  ...
  -> optional no-cache embedding retrieval over formal @aggregation entities
  -> model aggregate-fit decision with deterministic directive validation
  -> per uncovered SourceNeed: raw-entity embedding and compact model selection
  -> harness merges aggregate and raw selections and expands only selected fields
...
The model port is provider-neutral. `AGENT_MODE=mock` is the default. `AGENT_API_URL`, `AGENT_API_KEY`, `AGENT_MODEL`, and `AGENT_TIMEOUT_MS` configure OpenAI-compatible Chat Completions without introducing a provider SDK. `AGENT_TIMEOUT_MS` bounds each external model or Graph step and defaults to 600 seconds; `AGENT_RUN_TIMEOUT_MS` pins the whole planning run to 3,600 seconds by default and must be at least the step bound. The same adapter accepts a decrypted workspace-scoped profile from the Model Service service boundary. For the official DeepSeek endpoint, each planning stage uses `POST /responses` with `text.format.type=json_schema`, a generated stage-specific object schema, and `reasoning.effort=low`; the configured Chat Completions URL is safely mapped to the sibling Responses endpoint. The adapter reads only the Responses message `output_text`, requires a completed response, and treats an `incomplete` response as a bounded failure. Every structured response remains untrusted and is parsed by the stage's authoritative Zod schema. Exploration uses three model calls when no formal aggregate candidate is viable, three when all aggregate-capable needs are covered, and four for mixed aggregate/raw selections. One shared repair remains available inside the five-call ceiling. The API key is used only in the HTTP Authorization header and is absent from prompts, traces, outputs, and browser state. Redirects, oversized envelopes, malformed JSON, unknown fields, over-limit keywords, fabricated candidate references, uninspected query entities, fabricated aggregate directives, unknown operators, bad ports, duplicate inputs, disconnected nodes, cycles, and over-limit DAGs are rejected.

The Model Service profile is stored durably per authenticated, owner-authorized workspace. Its API key is encrypted before SQL using AES-256-GCM and a server-only versioned keyring; the API key and encryption metadata are never returned or written to browser storage. Saving the profile does not call the model. An explicit connection test sends one minimal fixed prompt with current form values, may incur provider charges, and does not save the profile or return provider content. The next explicit source-exploration operation uses the saved profile for its three bounded model decisions. Non-Agent creator actions reuse validated state so they cannot create hidden model charges. Public demo routes cannot read or change model profiles. Exact durable planning-call audit binding, explicit profile revocation, and distributed abuse controls remain pending.

When `EMBEDDING_ENABLED=true`, the API uses the server-only `EMBEDDING_API_URL`, `EMBEDDING_API_KEY`, and `EMBEDDING_MODEL` to rank inspected query entities before the entity-selector model call. The retriever embeds one semantic source-need description and bounded schema documents containing actual entity and field names, calculates cosine similarity inside Sprue, and supplies only compact scores plus entity references to the planner. Every source-need and entity document has an independent conservative UTF-8 byte budget below the configured provider's token ceiling; required facts, semantic constraints, suggested bindings, and direct fields are retained before lower-priority schema paths, with an explicit omitted-count marker. It keeps candidate diversity, does not relax network or schema validation, performs no Graph data query, and does not cache vectors or embedding responses. An enabled but unavailable or malformed embedding service fails explicitly rather than silently changing selection behavior. With the switch disabled, the existing deterministic ranking remains available.

After compact entity selection, Sprue runs a second semantic retrieval pass over every field of only the selected entities. Each field is an independent bounded embedding document; every declared semantic field requirement is an independent query document. Cosine similarity is computed locally, and the source-feasibility call receives the union of the highest-ranked alternatives per requirement plus compatible inspected binding evidence. There is no fixed global field-count prefilter before this pass, so retained evidence grows with the request's declared field requirements rather than an arbitrary schema slice. Each entity reports its complete and omitted field counts. The complete inspected entity remains inside the Harness: a model may bind only a path shown in its retrieved view, and the Harness validates that path and type again against the complete schema before accepting the composition. When embedding is disabled, the prior bounded deterministic relevance view remains the fallback. Neither path hardcodes a protocol, asset, network, query entity, or provider field name.

The user-visible trace exposes `aggregate_schema_retrieval` and `aggregate_selection` before the raw `semantic_entity_retrieval` and `source_entity_selection` fallback stages, then exposes `semantic_field_retrieval` before feasibility. A fully aggregate-covered plan records the raw fallback stages as skipped. Real provider-batch callbacks update stable in-progress events, local cosine calculation is reported as a short substate, and terminal events record bounded counts without vectors, provider response bodies, credentials, or hidden model reasoning. An embedding failure terminates the active retrieval stage explicitly.
...
backend/src/modules/agent/harness/
  ...
  entity-embedding.ts # Optional bounded OpenAI-compatible schema retriever; no cache
...
The deterministic wallet fixture path still runs for legacy evaluator coverage. The live exploration path is now `intent -> dynamic model SourceNeed/search plan -> per-need restricted Graph MCP search/activity/schema discovery -> optional embedding retrieval over actual inspected entity fields -> compact model entity selection -> harness expansion of selected entity fields -> exact required and auxiliary model field bindings plus generic operator plan -> deterministic validation`. Regression coverage proves that full fields are absent from entity-selection model input, embedding similarity can reorder opaque entity names using their schema contents, only selected entities are expanded for feasibility, an unfamiliar inspected provider field can be added as a purpose-bound filter input without a vocabulary match, and a non-wallet time-series result can use arbitrary semantic field names with `utc_date`, `sum`, and `average` composition. Next persist durable source snapshots, resolve a real gateway Deployment ID without conflating it with the returned manifest IPFS CID, compile the selected bindings into a bounded static GraphQL plan, and add an explicitly authorized data probe plus planning checkpoints/audit binding. Separately authorized live Graph execution follows after those gates.
...
On 2026-09-10, Sort / Top K became the eighth approved operator and superseded the seven-type allowlist above. Its version-1 contract uses predecessor-derived scalar fields, one to eight stable prioritized keys, explicit direction and null placement, and an optional K capped at 10,000. This does not approve unrelated remaining H1 schemas or change the live-source execution gates.
