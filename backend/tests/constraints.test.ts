...
test("database transaction, lineage and recovery contracts", async (t) => {
  ...
  try {
    ...
    const manifestSource = await insert("source_snapshots", {workspace_id:a.workspace, provider:"the_graph", source_kind:"subgraph", logical_source_id:"test-logical-subgraph", gateway_target_type:"manifest_ipfs_cid", gateway_target_id:"QmTestManifest", manifest_ipfs_cid:"QmTestManifest", data_network_ref:"test-network", schema_format:"graphql_sdl", schema_document:"type Query { items: [String!]! }", schema_hash:"test-manifest-schema-hash", discovery_method:"graph_mcp", status:"validated", observed_at:now, validated_at:now});
    assert.equal(manifestSource.gateway_target_type, "manifest_ipfs_cid");
