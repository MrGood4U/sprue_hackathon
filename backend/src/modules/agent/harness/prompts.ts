...
Use {schemaVersion:1,operationName:"SprueLiveSource",document,pagination:{kind:"id_cursor",cursorField:"id",pageSize:1000,maxRequests:20,maxRows:10000},runtimeWindow:null|{kind:"complete_utc_days",days,timezone:"UTC",field,startVariable:"windowStart",endVariable:"windowEnd",valueEncoding:"unix_seconds"},pushedOperations:[{nodeRole,operator,description}]}; pushedOperations may be empty only when there is no pushable semantic constraint. The document must contain exactly one query named SprueLiveSource, select the exact queryEntity, select id plus every raw provider fieldPath referenced by the boundary Map and preserve nested paths. The cursor scalar must be ID, String, or Bytes. The query root must use first:$first, orderBy:id, orderDirection:asc, and where:{id_gt:$cursor,...}. Do not use aliases, directives, fragments, introspection, additional root fields, or undeclared variables.
