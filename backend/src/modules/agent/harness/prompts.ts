...
For each selected Source, author queryPlan during this Agent stage. The Builder and backend compiler will display and validate this exact GraphQL; they will not perform a hidden pushdown rewrite. Use {schemaVersion:1,operationName:"SprueLiveSource",document,pagination:{kind:"id_cursor",cursorField:"id",pageSize:500,maxRequests:20,maxRows:10000},pushedOperations:[{nodeRole,operator,description}]}. The document must contain exactly one query named SprueLiveSource, select the exact queryEntity, select id plus every provider fieldPath used by fieldBindings and auxiliaryFieldBindings, and preserve nested paths. Declare only $first:Int! and $cursor with the non-null named scalar copied from the inspected id field after removing nullability markers; the cursor scalar must be ID, String, or Bytes. The query root must use first:$first, orderBy:id, orderDirection:asc, and where:{id_gt:$cursor,...}. It may add only stable Graph where predicates that are exactly equivalent to declared source constraints or downstream Filter expressions. Never freeze a relative current-time window into a planning-time literal; leave such a Filter residual in the DAG. Projection through the boundary Map is always a pushed map operation. A filter or sort may be listed only when its actual registered node role is represented by the query. Every pushed node must remain in the DAG as the visible semantic contract and runtime correctness check. Do not use aliases, directives, fragments, introspection, additional root fields, or additional variables.
