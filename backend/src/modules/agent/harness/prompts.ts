...
For each selected Source, author queryPlan during this Agent stage. The Builder and backend compiler will display and validate this exact GraphQL. Return the complete semantic composition so the controller can validate equivalence first; after validation, Sprue creates the residual Builder DAG by moving each declared pushed operation into Source. Direct provider-field bindings represented by the query projection move out of the boundary Map, so that Map retains only derived expressions or conversions; a Map with no residual fields is removed and its edges are rewired. List a Filter only when its entire stable predicate is represented by the GraphQL where object, because a listed Filter is removed from the residual DAG. List a Sort only when its complete semantics are represented by the allowed provider ordering, because a listed Sort is removed too. Never list a partial Filter or Sort, never list a transformation that GraphQL does not perform, and never freeze a relative current-time window into a planning-time literal. Use {schemaVersion:1,operationName:"SprueLiveSource",document,pagination:{kind:"id_cursor",cursorField:"id",pageSize:500,maxRequests:20,maxRows:10000},pushedOperations:[{nodeRole,operator,description}]}. The document must contain exactly one query named SprueLiveSource, select the exact queryEntity, select id plus every provider fieldPath used by fieldBindings and auxiliaryFieldBindings, and preserve nested paths. Declare only $first:Int! and $cursor with the non-null named scalar copied from the inspected id field after removing nullability markers; the cursor scalar must be ID, String, or Bytes. The query root must use first:$first, orderBy:id, orderDirection:asc, and where:{id_gt:$cursor,...}. Do not use aliases, directives, fragments, introspection, additional root fields, or additional variables.

When feasible, return {schemaVersion:2,kind:"source_feasibility",selections:[{sourceNeedId,candidateRef,queryEntity,fieldBindings:[{requirementId,fieldPath}],auxiliaryFieldBindings:[{name,fieldPath,purpose}],queryPlan:{schemaVersion:1,operationName:"SprueLiveSource",document,pagination:{kind:"id_cursor",cursorField:"id",pageSize,maxRequests,maxRows},pushedOperations:[{nodeRole,operator,description}]},rationale}],composition:{schemaVersion:2,kind:"composition_intent",nodes:[{role,operator,operatorVersion,config}],connections:[{fromRole,toRole,inputRole}],templateInstances:[]},assumptions:[]}.
