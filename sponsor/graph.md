...
The [advanced Subgraph feature guide](https://thegraph.com/docs/en/subgraphs/developing/creating/advanced/) defines formal timeseries and aggregation SDL: a timeseries entity uses `@entity(timeseries: true)`, an aggregate entity declares `@aggregation(intervals: ..., source: ...)`, and aggregate fields declare `@aggregate(fn: ..., arg: ...)`. Sprue recognizes only these directives as provider-authored prebuilt aggregation evidence. During Agent planning it ranks such aggregate entities before ordinary entities, validates the model's chosen interval/dimension/measure bindings against inspected SDL, and falls back independently per source need. It does not infer an aggregate from naming conventions, and provider field normalization remains an explicit Map.
