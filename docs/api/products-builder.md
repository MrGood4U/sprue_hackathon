...
| GET | `W/agent-sessions/{sessionId}/trace-events` | `afterSequence=0`, `limit=100` (max 100) | 200 `ActivePlanningTrace` | Cursor polling of the currently open, workspace-scoped planning trace |
...
The implemented `proposal` projection contains the model-produced intent summary, remote model identity/call count, real discovered Subgraph evidence (network, logical ID when available, manifest CID, query entity, matched facts, observed 30-day query count, rationale, and limitations), registered-operator composition counts, deterministic issues, persisted sanitized trace summaries, and `builderDraft`. `builderDraft = {schemaVersion:1,status:"requires_source_admission",sources,nodes,edges,outputSchema,refreshPolicy}` is a server-owned display/edit handoff assembled only from Harness-validated selections and composition: the model does not author a second browser-specific graph. Source records retain candidate/need identity, network, manifest CID, selected query entity, exact field bindings, and evidence status; source nodes plus allowlisted operator nodes retain stable role IDs and typed ports. It is not `DataProductSpec`, has no query document/access binding/source snapshot, and cannot be accepted, built, or executed. The proposal therefore still has `specification: null` and `readyForCompilation: false`: immutable Deployment ID resolution, exact coverage validation, access binding, and durable source-snapshot admission are required first. Unsupported proposals set `builderDraft:null`. The frontend may edit this explicitly non-executable planning draft, but must not relabel it as a compiled DAG or runtime result. While the initial handler remains synchronous, it appends trace events during execution and the authenticated session trace route supports cursor polling of the open stream. The same API process exposes the active command ID with that stream and supports cooperative cancellation of its in-memory run controller. Cancellation marks the durable command as requested, aborts the shared model/Graph run signal, and lets the original handler persist the terminal cancelled message, command, and retained trace. A process restart or a request routed to another API instance cannot recover that in-memory controller and returns a non-cancellable conflict; queue-backed cross-instance cancellation and SSE remain target behavior.
...
The proposed [planner orchestration](../../backend/harness/planner-orchestration.md), [harness workflow](../../backend/harness/workflow.md), and [tool catalog](../../backend/harness/tools.md) specify the internal implementation boundary for staged semantic interpretation, inspected-source selection, query compilation, primitive-DAG assembly, and Builder handoff. They add no arbitrary browser tool-execution endpoint; their exact H1-H3 schemas and limits remain under review alongside this API draft. The seven-type scope includes explicit Union/Join composition; semantic templates are compile-time authoring aids, not runtime types or arbitrary browser tools.
