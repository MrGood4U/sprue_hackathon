...
Draft 0.9. Read the [shared contract](../../api-contract.md) first. All paths below include `/api/v1`. `w` means workspaceId; IDs are UUIDs unless explicitly provider identifiers. Except app-config, every operation requires the verified creator. Workspace operations additionally require the active owner. Bootstrap wallet provisioning, the combined wallet read, Graph credential create/list/validate/select/revoke, and explicit Hedera testnet account activation are implemented. Complete mapped Hedera testnet accounts now project verified control and HBAR spend capability for the demonstrated Privy EVM path. Other command operations use approved M1/model 1.12 records and still require service implementation; provider control operations also depend on E1.
