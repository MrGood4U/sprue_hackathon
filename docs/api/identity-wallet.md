...
Draft 0.1. Read the [shared contract](../../api-contract.md) first. All paths below include `/api/v1`. `w` means workspaceId; IDs are UUIDs unless explicitly provider identifiers. Except app-config, every operation requires the verified creator. Workspace operations additionally require the active owner. Command operations depend on model gate M1; provider control operations also depend on E1.
