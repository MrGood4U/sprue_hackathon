...
| POST | `/api/v1/bootstrap` | `{}`; Idempotency-Key | 200 `Bootstrap` | Resolve or create auth identity + stable user + workspace + owner membership transactionally; no payment or wallet provision |
...
The Creator Console offers Google OAuth, GitHub OAuth, and MetaMask wallet login through one Privy application. Each successful method produces a provider-signed subject used only as an `auth_identities` lookup key. The binding resolves to the application-owned UUID returned as `Bootstrap.user.id`; the browser cannot select that user or a workspace ID. Multiple explicitly linked identities may resolve to one Sprue user, but different provider subjects are never merged by local email or wallet heuristics. Account linking, conflict handling, and unlinking require a separate reviewed flow. MetaMask authentication proves control of the connected login address through Privy's wallet-login flow. It does not automatically create, synchronize, fund, or delegate the separate account wallet used for bounded Graph spending. The public consumer route remains available without creator authentication.
