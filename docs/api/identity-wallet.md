...
The Creator Console offers Google OAuth, GitHub OAuth, and MetaMask wallet login through one Privy application. Each successful method produces a provider-signed Privy user subject and uses the same Sprue bootstrap contract; the browser cannot select a local user or workspace ID. Different provider subjects are not merged by local email or wallet heuristics; account linking requires a separate reviewed provider flow. MetaMask authentication proves control of the connected login address through Privy's wallet-login flow. It does not automatically create, synchronize, fund, or delegate the separate account wallet used for bounded Graph spending. The public consumer route remains available without creator authentication.
