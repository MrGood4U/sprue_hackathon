...
| POST | `/api/v1/public/demo/actions` | Run one bounded evaluator action: `build`, `api_request`, or `consumer_request`. | 200 |
...
The `parameters` object is currently optional and the accepted MVP values are fixed to the reviewed cross-chain example. An action response contains `{ "state": ..., "result": ... }` under `data`. The response metadata uses `dataSource: "demo"` and is not evidence of live Graph, Privy, Hedera, or Blocky402 settlement.
