...
The proposed interface design is [api-contract.md](api-contract.md) Draft 0.3 with four domain references under `docs/api/`. M1-M3 directions are approved and mapped to data-model 1.4; review route/DTO schemas and implement their service transactions before connecting frontend clients. E1/E2 remain gates for live provider and capped-consumer behavior; the document itself enables no payment or fee.

The proposed Agent/runtime preparation is [backend/harness/README.md](backend/harness/README.md) Draft 0.3. It defines natural-language clarification, source/schema verification, bounded GraphQL compilation, predefined operator composition, validation/simulation, and a separate creator-authorized build. Prepare the typed tool/script kit, registry and golden tests before wiring the model. H2 persistence directions are approved and the database foundation exists; H1 exact executable schemas and H3 source/methodology/operating limits remain open. The harness itself is still design-only, without scripts, controller or live integrations.
...
The first backend slice is [database.md](backend/database.md): 15 checksummed forward migrations, 51 tables/699 columns, nine domain schema files, explicit public network/HBAR seeds, a database-only Compose profile and offline transaction tests. The subsequent [backend framework](backend/framework.md) adds API/standby-worker process entries, health/readiness, public app configuration, verifier-gated identity reads, 100 domain route registrations, safe transport middleware and an explicitly partial OpenAPI artifact. Other domain handlers remain unavailable; no Agent, queue relay or provider action is implemented. Native PostgreSQL 17 and multi-connection verification remain pending because the local Docker engine was unavailable.
...
Frontend implementation is authorized using the accepted page architecture and selected visual direction. The existing frontend is the application baseline; remaining design reviews continue alongside it. Approved [data-model.md](data-model.md) version 1.4 is its persistence baseline. Initial migrations, typed query mappings and isolated SQL tests now exist under backend/; implementation evidence may still require an explicit model revision through the same change-control process.
