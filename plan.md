...
The proposed interface design is [api-contract.md](api-contract.md) Draft 0.6 with domain references under `docs/api/`. M1-M3 directions remain mapped to data-model 1.6; M4 durable model-profile persistence remains under review. Provider-independent creator identity is implemented, while account linking remains unimplemented. Review route/DTO schemas and implement their service transactions before connecting durable frontend clients. E1/E2 remain gates for live provider and capped-consumer behavior; the document itself enables no payment or fee.

The Agent/runtime preparation is [backend/harness/README.md](backend/harness/README.md) Draft 0.7. It defines natural-language clarification, bounded multi-source discovery/schema verification, per-source GraphQL compilation, explicit Union/Join composition, validation/simulation, and a separate creator-authorized build. The target [planner orchestration](backend/harness/planner-orchestration.md) uses three narrow model passes around deterministic source/query/spec compilation and returns only a validated canonical schemaVersion 2 specification to the Builder. The fixture-backed controller still validates proposals and runs the cross-chain transformation path with the mock planner by default or an explicitly configured OpenAI-compatible planner in the evaluator session. The multi-stage controller, durable Agent sessions, metering, queue execution, Graph adapters, and other live integrations remain open. H2 persistence directions are approved; H1 exact executable schemas and H3 source/methodology/operating limits remain open.
...
The first backend slice is [database.md](backend/database.md): 16 checksummed forward migrations, 52 tables/705 columns, nine domain schema files, explicit public network/HBAR seeds, a database-only Compose profile and offline transaction tests. The subsequent [backend framework](backend/framework.md) adds API/standby-worker process entries, health/readiness, public app configuration, verifier-gated identity reads, domain route registrations, safe transport middleware and an explicitly partial OpenAPI artifact. Provider subjects now resolve through many-to-one authentication bindings to stable Sprue user UUIDs. The evaluator demo routes and session-scoped model adapter are implemented, but account linking, durable domain handlers, queue relay, Graph/Privy/Hedera adapters, and payments remain unavailable. Native PostgreSQL and the Windows Docker profile have passed the documented local checks; cloud deployment and multi-connection race verification remain pending.
...
Frontend implementation is authorized using the accepted page architecture and selected visual direction. The existing frontend is the application baseline; remaining design reviews continue alongside it. Approved [data-model.md](data-model.md) version 1.6 is its persistence baseline. Forward migrations, typed query mappings and isolated SQL tests exist under backend/; implementation evidence may still require an explicit model revision through the same change-control process.
...
| 2026-09-07 | Added shared inline product-name editing and the `New Product` evaluator default | Keep the Dashboard row and product header synchronized through one session-scoped backend projection while preserving durable product creation and revision-safe renaming as explicit follow-up work |
