...
Version 1.3 was approved on 2026-09-05. It combines the previously approved defaults and Privy control model with current official Graph and Hedera refinements: dual Graph access, immutable source identity/provenance, per-query records, Hedera account-ID/address resolution, per-asset receive/access capability, x402 v2 `exact` requirements, Blocky402 capability snapshots, and normalized network settlement evidence. The human team also selected Hedera testnet HBAR (`0.0.0`) for the initial downstream integration and default demo path. This historical version was subsequently extended by version 1.4's M1-M3/H2 persistence directions, version 1.5's multi-Subgraph composition boundary, version 1.6's provider-independent Sprue user identity, version 1.7's workspace tenant boundary, version 1.8's encrypted model profile, version 1.9's encrypted Graph credentials plus authenticated Privy wallet provisioning, version 1.10's explicit idempotent Hedera testnet activation command, version 1.11's Graph credential validation, workspace-default selection, and secure revocation, version 1.12's complete-account control projection for the demonstrated Privy-to-Hedera testnet EVM path, version 1.13's creator-confirmed direct testnet withdrawal boundary, and version 1.14's durable product tombstone. The current database foundation implements 54 tables and 731 columns; account linking, durable withdrawal history and reconciliation, delegated wallet authority, Graph-key rotation, live source-specific query compatibility, and remaining service-level checks remain validation gates.
...
The human team approved the initial seven-page information architecture, interaction specification, capped demo-consumer direction, and large-screen browser scope on 2026-09-05. The product is a web application, not a Windows or macOS native client. Mobile and tablet-specific layouts are deferred; the structured DAG editor remains a browser-based keyboard and single-pointer alternative. The human team then selected the third visual exploration, Evidence-First Console. The maintained application subsequently split Agent planning from DAG editing, added a top-level Model Service page, separated public Entry from Creator Login, moved authenticated account access into the shared top-right Creator Console headers, colocated Dashboard product creation with the product collection, and added shared inline product-name editing, producing the current ten-page architecture in product-design 1.38. Route pages, application composition, shared components, feature components, and backend-demo data now have explicit file owners instead of sharing one multi-page module. A formal three-layer design-token Draft 0.1 defines the dark MVP palette, meaningful accent roles, typography, spacing, component states, layout contracts, accessibility checks, and generated CSS workflow. DT1-DT4 remain review follow-ups, and durable product creation/naming, account-linking UI, explicit model-profile revocation/audit binding, plus the capped demo-consumer security/funding boundary remain gates for production behavior.

### 6. MVP Implementation — Current

Continue developing the existing ten-page frontend directly. Route-level business data comes from the explicit backend demo runtime; maintain feature hooks and service contracts so durable backend resources can replace it incrementally. Current frontend gaps and their implementation order are recorded in [frontend/implementation-status.md](frontend/implementation-status.md).

The proposed interface design is [api-contract.md](api-contract.md) Draft 0.12 with domain references under `docs/api/`. M1-M4 directions are mapped to data-model 1.13; durable encrypted Model Service resources, Graph credential create/list/validate/select/revoke, authenticated user-wallet provisioning, current Privy and Hedera balance reads, explicit Hedera testnet account activation, complete-account control projection for the demonstrated Privy EVM path, and creator-confirmed direct Base Sepolia USDC/Hedera testnet HBAR withdrawals are implemented. Provider-independent creator identity and workspace-isolated creator routes are implemented, while account linking, durable withdrawal history and reconciliation, delegated spending authority, Graph-key rotation, source-specific query compatibility validation, exact planning-call audit binding, and explicit model-profile revocation remain unimplemented. The remaining E1/E2 items remain gates for paid provider and capped-consumer behavior; the document itself enables no autonomous payment or fee.

The Agent/runtime preparation is [backend/harness/README.md](backend/harness/README.md) Draft 1.0. It defines natural-language clarification, bounded multi-source discovery/schema verification, per-source GraphQL compilation, explicit Union/Join composition, validation/simulation, and a separate creator-authorized build. The target [planner orchestration](backend/harness/planner-orchestration.md) first asks the workspace-configured model for strict semantics and bounded keyword hints, invokes the restricted Graph MCP adapter through controller-owned methods, then asks the model to assess inspected candidates and propose a registered-operator topology. Deterministic checks return only a non-executable feasibility result until durable source admission is complete. The fixture-backed compatibility controller separately validates canonical proposals and runs the cross-chain transformation path. Live workspace Graph-credential binding, durable source snapshots, Deployment ID resolution, exact field mapping, coverage validation/execution, durable Agent sessions, metering, queue execution, and other live integrations remain open. H2 persistence directions are approved; H1 exact executable schemas and H3 source/methodology/operating limits remain open.
...
The first backend slice is [database.md](backend/database.md): 21 checksummed forward migrations, 54 tables/731 columns, domain-split schema files, explicit Base Sepolia USDC and Hedera HBAR seeds, a database-only Compose profile, and offline transaction tests. The subsequent [backend framework](backend/framework.md) adds API/standby-worker process entries, health/readiness, public app configuration, verifier-gated identity reads, domain route registrations, safe transport middleware, durable encrypted model-profile and Graph-credential lifecycle handlers, authenticated Privy wallet provisioning, current Privy and Hedera balance adapters, explicit idempotent Hedera testnet activation, complete-account control projection for the demonstrated Privy EVM path, durable workspace-scoped product creation/rename/delete, and generated OpenAPI. Provider subjects resolve through many-to-one authentication bindings to stable Sprue user UUIDs. Creator-private reads and writes are isolated by the authenticated workspace; the frontend additionally supports creator-confirmed direct testnet withdrawals as browser-to-Privy actions. Account linking, queue relay, durable withdrawal history and reconciliation, delegated spending authority, live source-specific Graph execution, native Hedera x402 signing/publication, and payments remain unavailable. Cloud deployment and multi-connection race verification remain pending.
...
On 2026-09-05, the human approved a small node scope and the corresponding repository changes; the current registry contains eight runtime types: source/filter/map/aggregate/sort/union/join/output. Wallet Activity and Repeat Activity are compile-time templates. The maintained frontend projects current live Agent drafts, and the backend now performs a read-only compilation preflight over the exact layout-free Builder DAG. It validates topology, operator contracts, and derived schemas but does not admit sources, persist versions, generate GraphQL, execute data, deploy an API, or create durable compilation evidence. H3 live source/methodology/limits and those executable boundaries remain open. H2 and M1-M4 directions remain persisted in model 1.8; E1/E2 provider/buyer gates remain unverified.
...
Store a versioned execution definition separately from UI layout and pin each run to a definition version. Queue scheduling, DAG execution, and result refresh/materialization are separate responsibilities. The technical baseline is selected above; the eight-type operator scope is now confirmed. The current read-only compiler validates registered versions, bounded configurations, Union compatibility, Join keys/cardinality/null/collision behavior, numeric/null semantics, and predecessor-derived schemas; persistence and executable runtime integration remain separate gates. The registry and compiler/tool constraints are recorded in [backend/harness/operators.md](backend/harness/operators.md).
...
Frontend implementation is authorized using the accepted page architecture and selected visual direction. The existing frontend is the application baseline; remaining design reviews continue alongside it. Approved [data-model.md](data-model.md) version 1.8 is its persistence baseline. Forward migrations, typed query mappings and isolated SQL tests exist under backend/; implementation evidence may still require an explicit model revision through the same change-control process.
...
| 2026-09-07 | Multi-stage planner orchestration and Builder DAG handoff | Refined the harness into three narrow model passes for semantic interpretation, inspected-source selection, and operator composition, surrounded by deterministic source/query/spec compilation; defined typed intermediate artifacts, source ranking, repair policy, canonical schemaVersion 2 assembly, and frontend projection rules | Human requested a design for turning natural-language product requirements into selected existing Subgraphs and a structured operator DAG before implementation | Cross-checked the canonical data model, API Proposal, operator/template contracts, current one-call harness, deterministic runtime, and maintained Builder codec; validated six JSON blocks, ten local document links, English-only text, and diff formatting. Documentation only: no runtime code, provider query, model call, source deployment, product version, payment, or deployment was created |
...
| 2026-09-07 | Privy creator authentication | Added explicit Google OAuth, GitHub OAuth, and MetaMask wallet login through one Privy frontend boundary; implemented server-side access-token verification, fail-closed creator routes, transactional idempotent local user/default-workspace bootstrap, account display, and sign-out | Human requested an account/login system using Google, GitHub, and MetaMask and asked for a dedicated backend folder | Current Privy documentation defined the OAuth, wallet-login, React, Node, and access-token boundaries. UI/UX guidance informed provider clarity, loading/error feedback, focus-visible controls, and preserving unauthenticated public access. The 43-test backend suite, complete 37-test frontend build, Docker rebuild/four-service readiness, and browser checks for the configured-off state, `/app` guard, and public `/p/*` route passed. Provider verification used injected fakes and isolated PostgreSQL; no real Privy credentials, live login, wallet creation, signing authority, Graph request, payment, or cloud deployment was used. The frontend production dependency audit retains 25 moderate and two high transitive advisories; a non-breaking fix was applied, while forced Privy downgrade was rejected pending compatibility review |
| 2026-09-07 | Provider-independent Sprue user identity | Moved provider subjects from `users` into many-to-one `auth_identities`, preserved application-owned user UUIDs through a forward backfill migration, and updated authentication/authorization lookup to carry an explicit provider key | Human required Sprue-owned unique IDs so Privy can be replaced and multiple Privy accounts can be linked without changing domain ownership | Updated data-model 1.6 before implementation, retained existing UUIDs, prohibited heuristic account merging, and left link/unlink/recovery commands unimplemented pending a reviewed proof flow. The 44-test backend suite, typecheck, migration repetition, model/schema parity, identity-backfill preservation, two-identities-to-one-user, identity-conflict, and revoked-identity regressions, native PostgreSQL migration, and Docker readiness passed. No real identity was linked, provider account changed, wallet action performed, or external request made |
| 2026-09-07 | Dedicated Creator Login route | Removed provider controls from the public Entry page, added a secondary header `Log in` action and primary hero `Enter console` action, and routed both to a dedicated `/login` page while signed out | Human explicitly requested that the landing page explain the product first and defer provider selection until the creator chooses to enter | UI/UX guidance informed the single primary hero action, secondary header action, predictable route transition, and keyboard focus behavior. The complete 37-test frontend build, four Sites tests, Docker image rebuild and four-service readiness passed. Browser checks confirmed both creator actions reach `/login`, provider choices are absent from Entry and present on Login, the configured-off state is explicit, and no console errors were emitted. No live provider login, identity link, wallet action, payment, or external request was performed |
| 2026-09-07 | Direct OAuth login completion | Removed MetaMask from the current creator login surface and Privy client configuration, kept Google and GitHub, and made a server-bootstrapped authenticated Login route navigate directly to the Dashboard without a signed-in confirmation card | Human narrowed the MVP to two OAuth methods and explicitly removed the extra post-login decision | Focused UI/UX guidance supported removing the redundant success destination while preserving initialization feedback and waiting for verified backend bootstrap. React guidance informed a dependency-complete redirect effect. A user-provided screenshot established a successful local Google session before this change. Thirty-five focused frontend tests, the Docker production build, local readiness checks, and direct `/login` and `/app` route probes passed; repeatable post-change external OAuth evidence remains pending. No wallet login, payment authority, Graph request, or account-linking behavior was added |
| 2026-09-07 | Official OAuth provider marks | Replaced accent-colored generic login glyphs with the official Google G and GitHub Invertocat assets, kept both provider actions at equal visual weight, and removed runtime image dependencies | Human requested the original Google and GitHub logos instead of Sprue-colored variants | Official provider guidance and focused UI/UX review informed unmodified brand color, high contrast, embedded asset use, decorative icon semantics, and stable sizing. The complete 37-test frontend suite, token validation, production frontend build, diff checks, and a browser screenshot of the disabled-provider state passed; no provider login or external identity change was performed |
| 2026-09-07 | Shared account header menu | Moved authenticated account access from the persistent sidebar into the right edge of both shared Creator Console headers and added an anchored identity/workspace/navigation/sign-out menu | Human requested a familiar top-right avatar control with a click-expanded menu based on the supplied references | Focused UI/UX guidance informed the native button and menu semantics, visible focus, arrow/Home/End navigation, Escape focus return, outside-click dismissal, and visual separation of sign-out. The complete 38-test frontend build, four Sites tests, token validation, Docker image rebuild and four-service readiness passed. The local browser route loaded without application console errors; external OAuth was not repeated and no identity, wallet, payment, or provider state changed |
| 2026-09-07 | Dashboard action grouping | Moved `New product` from the page header into the `All products` toolbar before search and filter, and aligned the remaining language/account controls to the top-right edge | Human identified that separating creation from the product collection split attention and requested the global controls move farther into the top-right corner | Focused UI/UX guidance informed object-action proximity, one clear collection action, and matching visual and keyboard order. The complete 38-test frontend build, four Sites tests, token validation, Docker image rebuild, migration check, and four-service readiness passed. The local `/app` route reached the expected authentication/desktop gate in a fresh browser session; external OAuth was not repeated and no backend business, identity, wallet, payment, or provider state changed |
| 2026-09-07 | Inline product naming | Added one controlled inline-name editor to the Dashboard product row and shared product header, made Dashboard editing pencil-triggered, made the product-header title and pencil editable, and assigned `New Product` before the evaluator creation flow opens the Agent view | Human requested direct title editing at both product recognition points, outside-click saving, and a predictable initial name for a new product | Focused UI/UX guidance informed controlled input state, blur/Enter commit, Escape cancellation, visible focus, an accessible persistent pencil action, and recoverable inline errors. The rename is a strict 1–120 character backend demo action scoped to one process-memory session and does not call the configured model. Forty-five backend tests, typecheck, backend build, OpenAPI checks, the complete 38-test frontend build, four Sites tests, Docker rebuild/migration/four-service readiness, and live same-session/cross-session HTTP checks passed. A fresh Codex browser had no reusable authenticated tab, so external OAuth and authenticated visual interaction were not repeated; no durable product row, provider request, wallet action, or payment was created |
| 2026-09-07 | Workspace account isolation | Promoted the data model to 1.7, required same-workspace membership for creator-attributed records, and moved evaluator creator state/actions/model profiles behind verified owner-scoped workspace routes | Human required wallet, product, and other account-owned data to remain inaccessible across accounts after authentication was added | The audit found that durable tables were workspace-keyed but creator actor references lacked a same-workspace database guard, while the temporary demo runtime trusted a browser-generated session UUID on public mutation routes. Migration 0017 adds defense-in-depth actor constraints; creator routes now require a verified bearer identity plus owner authorization, foreign workspace lookups use the non-disclosing not-found boundary, and only immutable public projection plus simulated consumer access remain public. Backend tests, typecheck/build, OpenAPI checks, frontend service tests, and the production frontend build passed. No live provider, wallet, payment, or durable product operation was performed |
| 2026-09-07 | Durable encrypted Model Service profiles | Promoted the data model to 1.8, added one owner-authorized model profile per workspace, encrypted API keys before PostgreSQL with AES-256-GCM and a server-only versioned keyring, and moved frontend reads/writes/tests to the durable resource | Human explicitly required the Model Service page to correctly store and retrieve saved language-model API keys | The implementation stores ciphertext, random IV, authentication tag, key ID, secret version, and keyed fingerprint while never returning raw or encrypted secret material. Saving is non-calling, testing is non-saving, blank-key updates retain and re-encrypt the current key, and the next Agent plan resolves it internally. Isolated tests cover restart persistence, rotation, tamper rejection, redaction, and cross-workspace denial; no real API key or provider call was used |
| 2026-09-07 | Authenticated wallet and Graph credential integration | Promoted the data model to 1.9, idempotently provisioned a user-owned Privy Ethereum wallet after verified account/workspace bootstrap, bound it to the Sprue user/workspace, read current Base Sepolia USDC through Privy, and added encrypted workspace Graph API-key create/list/resolve services plus the live Wallet and Access frontend | Human required Wallet and Access to use account-owned backend data, provision a Privy wallet for new accounts, show the real Graph-funding balance, and correctly store Graph API keys | Current Privy wallet create/list/balance and Graph gateway/API-key documentation informed the provider boundary. Tests cover exact SDK arguments, owner-authorized HTTP routing, idempotent binding, balance projection/snapshots, encryption/redaction/restart/tamper behavior, and cross-workspace denial. The 60-test backend suite, strict build/OpenAPI checks, and full frontend production build pass. No real wallet was created, funded, signed, or charged; no real key, live Graph query, Hedera recipient, or payment was used |

| 2026-09-12 | Independent Hedera x402 consumer | Implemented the standalone `hx402-cli` package with encrypted ECDSA wallet creation/import, Mirror Node account resolution and balance reads, optional user-PAT testnet faucet funding, exact native-HBAR policy inspection, approval-bound standard x402 signing and protected response delivery; corrected Sprue's payment decoder to accept the standard v2 payload shape | Human requested a flexible buyer CLI outside Sprue so a faucet-funded account can call arbitrary Hedera x402 APIs | Ten CLI tests construct and decode an SDK-signed payload without submission, reject requirement drift, and cover key encryption, exact HBAR conversion, Mirror Node resolution, and the fixed Portal request boundary; the dependency audit, full Sprue backend suite, builds, and strict typechecks pass. No faucet request, funded payment, settlement, or creator receipt was performed. |
| 2026-09-13 | Seven-language Creator Console and current submission documentation | Extended the maintained message catalogs, language switcher, persisted locale handling, and Agent response-locale contract to English, Simplified Chinese, Spanish, French, German, Korean, and Japanese; refreshed the README, MVP flow, deployment boundary, implementation status, and sponsor references to match the completed demo path | Human requested the five additional locales and then asked for the previously identified documentation updates before repository submission | The frontend production build and full frontend test suite passed, including catalog-key, placeholder, translation, and native-language-name checks. Backend typecheck, OpenAPI artifact check, 175 tests, production build, and compiled API/worker smoke checks passed. The staged content was scanned for private keys, token-shaped credentials, and secret assignments before commit. |
...
| Date | Change | Reason |
|---|---|---|
| 2026-09-13 | Expanded the Creator Console and Agent response contract to seven locales and reconciled public project documentation with the implemented hackathon MVP | Ensure every advertised language is selectable end to end and keep judge-facing setup, workflow, deployment, and sponsor evidence accurate rather than preserving superseded planning claims. |
| 2026-09-12 | Added an interactive `hx402-cli>` terminal and a reproducible standalone Windows executable build | Let a buyer start `hx402-cli.exe` once and perform multiple wallet, faucet, quote, and paid-request operations while retaining the existing one-shot automation interface and client-side secret boundaries. |
| 2026-09-12 | Renamed the public x402 consumer command to `hx402-cli` and added guided, topic-specific help | Keep the executable name aligned with the package purpose and make first-time wallet setup, faucet funding, payment inspection, and paid requests discoverable from both one-shot and interactive modes. |
...
| 2026-09-10 | Added independent byte-bounded truncation for every embedding input document | Keep large inspected schemas within the embedding provider's per-input token ceiling without reducing candidate coverage. Required source facts, constraints, suggested bindings, and direct fields retain priority, while omitted lower-priority details remain explicit. |
...
| 2026-09-07 | Added Google, GitHub, and MetaMask creator login through Privy with backend-verified account/workspace bootstrap | Establish one secure creator identity path while keeping login separate from account-wallet creation, funding, and payment authority |
| 2026-09-07 | Separated Sprue user IDs from authentication-provider subjects | Preserve stable application ownership across provider replacement and permit future explicit many-to-one account linking without heuristic merges |
| 2026-09-07 | Separated public Entry from Creator Login and routed both creator entry actions through `/login` while signed out | Keep product explanation and authentication-provider selection as distinct decisions while preserving direct console access for an existing session |
| 2026-09-07 | Limited Creator Login to Google and GitHub and redirected successful bootstrap directly to the Dashboard | Keep the MVP login surface focused and remove a redundant post-authentication choice without bypassing backend identity initialization; 35 focused frontend tests, the Docker production build, local readiness checks, and direct `/login` and `/app` route probes passed |
| 2026-09-07 | Replaced theme-colored login glyphs with official Google and GitHub brand marks | Improve provider recognition and preserve approved provider branding without changing authentication behavior |
| 2026-09-07 | Moved authenticated account access from the sidebar into a shared top-right avatar menu | Follow a familiar account pattern, recover sidebar space, and keep identity, current workspace, existing destinations, and sign-out in one keyboard-accessible disclosure |
| 2026-09-07 | Colocated Dashboard product creation with the `All products` toolbar and top-aligned global header controls | Keep the creation action attached to the collection it changes while preserving a stable top-right location for language and account access |
| 2026-09-07 | Added shared inline product-name editing and the `New Product` evaluator default | Keep the Dashboard row and product header synchronized through one authenticated workspace-scoped backend projection while preserving durable product creation and revision-safe renaming as explicit follow-up work |
| 2026-09-07 | Enlarged and strengthened the Builder canvas dot grid | Improve spatial orientation with a dedicated higher-contrast DAG token and wider spacing while keeping nodes and lineage edges visually dominant |
| 2026-09-07 | Kept the Builder grab cursor consistent across nodes, ports, and edges in hand mode | Match the visual affordance to the active pan-only tool while preserving normal cursors for floating controls |
| 2026-09-07 | Separated Graph-funding USDC from x402-revenue HBAR and added balance-scoped outbound transfer entry points | Keep network and asset meaning explicit while the evaluator UI fails closed until M5/E1 signing, submission, and reconciliation are reviewed and implemented |
| 2026-09-07 | Added workspace account isolation to the data model and evaluator runtime | Prevent authenticated creators from reading or mutating another workspace's wallet, product, model profile, or creator state through either durable actor references or temporary demo routes |
| 2026-09-07 | Added durable encrypted workspace model profiles | Preserve creator model configuration across API restarts without exposing API keys to PostgreSQL plaintext, browser storage, logs, or API responses |
| 2026-09-07 | Implemented restricted Graph MCP source discovery for the Agent harness | Allow bounded existing-Subgraph keyword or creator-supplied contract discovery while denying all MCP query-execution tools to planning; require 30-day activity checks, inspect immutable manifest-CID schemas, and rank field/network evidence deterministically | Added the official MCP client behind a provider-neutral port, strict provider-result validation and redaction, GraphQL SDL inspection, a harness discovery entry point, and regression tests. The full backend suite and strict TypeScript validation passed. No live credential/provider call, data query, payment, durable source snapshot, or source selection was performed |
| 2026-09-07 | Ordered Agent reasoning around Graph MCP discovery | Ask the model first for strict semantics and minimal keyword hints, let the controller run only fixed metadata capabilities, then ask the model whether returned source evidence and registered operators can form a viable topology | Added strict discovery-plan and source-feasibility schemas/prompts, the `AgentHarness.explore` controller path, deterministic keyword/candidate/schema/topology validation, and an ordering/security regression test. Results remain non-executable pending workspace Graph credential binding, immutable source admission, exact field mapping, coverage and access checks; no live provider call, data query, payment, product version, or deployment occurred |
| 2026-09-07 | Connected Wallet and Access to authenticated workspace resources | Replace sample wallet/credential values with an idempotently provisioned Privy wallet, current Base Sepolia USDC observations, and encrypted Graph API-key storage | Added migration 0019, provider/domain/repository modules, owner-authorized HTTP/OpenAPI routes, the live frontend client/page states, and regression coverage. Wallet signing, bounded spending, Graph-key validation/use, Hedera revenue, transfers, and paid requests remain separate gates |
| 2026-09-07 | Added bounded Privy wallet retries and redacted provider diagnostics | Make transient first-login wallet provisioning failures diagnosable and recoverable without retrying permission or request errors or exposing identity and credential data | Disabled hidden SDK retries in favor of one explicit bounded retry for connection, timeout, rate-limit, and 5xx failures; retained a stable wallet-create idempotency key, added categorized structured logs, extended the bootstrap request window, and added regression coverage. No transaction or fund movement is authorized by this behavior |
| 2026-09-07 | Repaired incomplete Privy wallet bindings and made reference-data initialization part of deployment orchestration | Prevent a missing Base Sepolia reference row from leaving a wallet marked ready without an address or balance | Made wallet/address persistence atomic, re-runs provider binding when the required active address is absent, reports incomplete bindings as blocked, and runs only the idempotent public network/asset seed after migrations in local and Railway release flows. No user, wallet, credential, balance or funds are seeded |
| 2026-09-08 | Added explicit Hedera testnet account activation for an authenticated workspace wallet | Let a creator request a canonical Hedera recipient without entering another address while preventing cross-workspace use and blind transfer retries | Reused the bound Privy EVM address, durable control-command idempotency, the fixed Portal faucet endpoint, Mirror Node reconciliation, Hedera address/capability rows and HBAR snapshots. The browser shows a disabled spinner state and retryable failures. Tests use provider doubles only; no faucet transfer was submitted, and signing/access to proceeds remains unverified |
| 2026-09-08 | Verified and retired the temporary Privy-to-Hedera signing spike | Convert the one-time development check into a stable complete-account readiness rule instead of asking every creator to repeat a fee-bearing test | The creator-confirmed Privy EVM transaction succeeded on Hedera testnet, completed account `0.0.10410307`, and paid `0.02163000 HBAR` in network fees. The temporary UI was removed; Mirror Node completion at the same bound EVM address now projects verified control and HBAR spend capability. General transfers, delegated spending, native x402 signing/settlement, and mainnet remain unavailable |
| 2026-09-08 | Added confirmed deletion for workspace data products | Give creators an explicit, recoverability-aware way to remove a product from active workspace use without breaking retained versions, runs, or evidence | Added a row-level trash action, product-named confirmation dialog, authenticated idempotent `DELETE` route with lock-version preconditions, and migration 0021 soft-delete tombstones. The first browser verification exposed a missing DELETE CORS preflight allowance; the transport allowlist and regression test were corrected, the local API was rebuilt, and a live preflight now returns 204 with DELETE explicitly allowed. No existing local product was deleted during verification |
| 2026-09-08 | Hardened remote Agent structured planning output | Prevent provider JSON-shape drift from turning a valid intent into an opaque early failure and preserve the real observed runtime after completion | Added strict structured-output contracts, one repair inside the four-call ceiling, a 16,384-token output budget, safe diagnostics, and persisted duration. Provider calls remain explicit and potentially billable; regression tests use injected responses and do not call the saved external model. |
| 2026-09-10 | Added optional embedding retrieval for inspected Subgraph entities | Compare large, unfamiliar provider schemas with each semantic source need before the selector model chooses references, without hard-coded protocol fields or full schema arrays in its prompt | Added API-only OpenAI-compatible embedding configuration, bounded batched entity-schema documents, local cosine ranking with fair candidate coverage, compact advisory scores, explicit failures, deterministic fallback when disabled, and no vector cache. Embeddings do not select sources, relax validation, query Graph data, or create executable authority. |
| 2026-09-10 | Added hierarchical semantic retrieval for selected Subgraph fields | Narrow large candidate sets at entity granularity first, then compare every field of only the model-selected entities against every declared semantic field requirement without a fixed global field prefilter | Added a second batched embedding pass, local field-to-requirement cosine ranking, per-requirement alternative unioning, complete-schema validation, deterministic fallback, and a dedicated live field-retrieval trace step. No vector cache, Graph data query, source-selection authority, or protocol/provider field hardcoding was added. |
| 2026-09-11 | Connected Builder Source search and direct identifier verification to the restricted Graph adapter | Replace inert modal placeholders with authenticated live catalog/activity search and exact schema inspection while keeping provider credentials server-only | Added workspace-scoped search/validate routes, strict live response contracts, bounded Graph MCP calls, explicit UI states and atomic planning-draft source insertion. Verification remains non-executable planning evidence pending immutable source admission, coverage, endpoint access, and runtime compilation. |
| 2026-09-11 | Made combined Map expressions inspectable and safely editable | Let creators understand and refine Agent-authored field logic without exposing an arbitrary-code surface or silently replacing the expression | Added readable formula and field/type metadata, a progressively disclosed bounded-JSON editor, backend-parity shape/depth/node/type validation, inline feedback, and protected Apply/Confirm behavior. |
| 2026-09-11 | Aligned Map definition controls independently of helper text | Keep the output field, transform selector, and row action visually stable when a combined expression adds explanatory copy | Top-aligned the field columns, aligned the delete action with the control row, and reset the desktop offset in the stacked mobile layout. |
| 2026-09-11 | Connected Builder to an authoritative read-only DAG compilation preflight | Validate the exact current graph before leaving Builder, expose request progress, and make failures actionable without discarding edits | Added bounded topology/operator/schema compilation, a canonical request hash, a disabled rotating action, a node-attributed failure dialog, and success navigation to API. Source admission, durable versions, GraphQL generation, data execution, and deployment remain separate gates. |
| 2026-09-11 | Prioritized formal Subgraph prebuilt aggregations during Agent planning | Avoid unnecessarily paging raw entities when provider-authored aggregate schemas already satisfy a requested grain and measure set | Discovery now preserves formal timeseries and aggregation directives, aggregate schemas are embedded and assessed before raw entities, exact intervals/functions/fields are deterministically validated, and raw entity retrieval runs only for uncovered source needs. Frontend progress reports aggregate assessment and raw fallback separately; Source normalization remains visible in Map. |
| 2026-09-12 | Simplified Wallet and API status presentation and added live Graph query metering | Remove non-actionable status summaries while giving creators one truthful view of provider load across private and paid access | Removed the Wallet readiness sidebar and API backend-status card. Successful live DAG executions now append the actual Graph page-request count to `usage_events`, tagged by API-key or x402 access mode, and the Data Products overview sums those events across the rolling 24-hour period. |
| 2026-09-12 | Promoted X402 invocation details and financial totals | Make paid-endpoint use and earned value immediately legible without opening a secondary modal or exposing implementation-oriented API labels | Added a full wrapping X402 URL and cURL challenge example with copy feedback, enlarged tabular HBAR totals, removed the API request tester's backend-provenance badge, and renamed the available endpoint state to `Deployment ready`. |
