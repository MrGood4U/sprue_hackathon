...
| 2026-09-08 | Hardened remote Agent structured planning output | Prevent provider JSON-shape drift from turning a valid intent into an opaque early failure and preserve the real observed runtime after completion | Added one bounded `submit_sprue_plan` function-call contract for generic OpenAI-compatible profiles, with stage schemas, full server-side validation, one repair attempt inside the three-call ceiling, and persisted/rendered duration. A live DeepSeek diagnostic exposed an immediate HTTP 400 because Thinking mode rejects explicit `tool_choice`; omitting that field returned a real tool call, but the provider's Responses API also supports the stricter `text.format.type=json_schema` contract. Official DeepSeek profiles now use `POST /responses`, bounded low-effort reasoning, an 8192-token output budget, and strict parsing of the Responses message output; `incomplete` responses are classified explicitly. The transport schema remains normalized to the provider-compatible subset and HTTP errors retain only safe provider code/type/parameter identifiers. Real diagnostics showed 4096 tokens can be consumed by reasoning before a result, while low-effort 8192-token Structured Outputs completed with valid JSON. Provider calls remain explicit and potentially billable; regression verification uses injected responses and does not call the saved external model. |
