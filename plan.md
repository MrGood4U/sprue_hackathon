...
Choose the implementation stack and validate the external dependencies that directly support the core flow. The selected sponsors are The Graph, Hedera, and Privy. The Graph supplies data purchased by Sprue on the creator's behalf. Privy provides the creator account wallet for funding and delegated Graph payments; the intended creator-controlled Hedera receipt path still needs validation. Sprue implements optional x402 v2 `exact` access for its hosted APIs, using Blocky402 to verify and settle downstream HBAR or HTS payments on Hedera. Validate the upstream payment path and downstream settlement independently; sponsor qualification still requires evidence.
...
Version 1.1 was approved on 2026-09-05. The human team accepted materialized-result serving, owner-only MVP authorization with future-ready membership records, bounded PostgreSQL JSON artifacts, retention of user-visible Agent records without hidden chain-of-thought, and the Privy model of a user-owned wallet with a policy-bound Sprue additional signer, immutable provider-policy snapshots, and Sprue database budget enforcement. Draft 1.2 was then prepared from current official Graph documentation, including the human-confirmed choice between a customer-supplied API key/existing subscription and creator-wallet x402 pay-per-query. Draft 1.3 now combines those pending Graph refinements with current official Hedera x402 requirements: account-ID/address resolution, per-asset receive/access capability, x402 v2 `exact` requirements, Blocky402 capability snapshots, and normalized network settlement evidence. The combined draft requires human review before affected migrations. Remaining live provider compatibility and implementation checks are tracked as validation gates.
...
The approved version 1.1 decisions in [data-model.md](data-model.md) are the baseline for this sequence. Review and accept or revise Draft 1.3 before generating source, provider-credential, wallet-capability, publication, or payment migrations; other implementation evidence may still require an explicit model revision through the same change-control process.
...
4. On Hedera testnet, choose HBAR or one HTS fungible token, resolve the intended creator recipient to a complete Hedera account ID, and validate its per-asset receive/access capability. Do not infer Privy compatibility from an EVM address or export user keys to adapt a sample.
...
- Verify creator control of a resolved Hedera account ID and receive/access capability for the selected asset; distinguish its balance from Base Graph-spending funds. A display-only EVM address mapping is insufficient evidence.
