...
The proposed interface design is [api-contract.md](api-contract.md) Draft 0.3 with four domain references under `docs/api/`. M1-M3 directions are approved and mapped to data-model 1.5; review route/DTO schemas and implement their service transactions before connecting frontend clients. E1/E2 remain gates for live provider and capped-consumer behavior; the document itself enables no payment or fee.

The Agent/runtime preparation is [backend/harness/README.md](backend/harness/README.md) Draft 0.5. It defines natural-language clarification, bounded multi-source discovery/schema verification, per-source GraphQL compilation, explicit Union/Join composition, validation/simulation, and a separate creator-authorized build. A fixture-backed mock controller now validates proposals and runs the non-model cross-chain transformation path; durable Agent sessions, scripts, queue execution and live integrations remain open. H2 persistence directions are approved; H1 exact executable schemas and H3 source/methodology/operating limits remain open.
...
On 2026-09-05, the human approved a small node scope and the corresponding repository changes, then expanded it to seven runtime types: source/filter/map/aggregate/union/join/output. Wallet Activity and Repeat Activity are compile-time templates. The maintained frontend still demonstrates the earlier single-source example, actual-edge rendering, semantic disclosure and local parameter recompilation; it does not yet demonstrate Union/Join. Data-model and API examples share a denominator-safe repeat-activity metric and four string-safe output fields. This is frontend/design progress, not an implemented Agent or backend runtime. H1 exact schemas and H3 live source/methodology/limits remain open. H2 and M1-M3 directions were subsequently approved and persisted in model 1.5; runtime behavior and E1/E2 provider/buyer gates remain unverified.
